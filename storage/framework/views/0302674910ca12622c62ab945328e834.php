...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
    ...
    <?php $__env->startSection('hide_global_errors', true); ?>
        <div id="alokasiSesiRoot" x-data="alokasiSesiApp()" class="w-full relative"
            data-open-modal="<?php echo e($errors->any() ? '1' : '0'); ?>"
            data-jadwals='<?php echo json_encode($jadwals ?? [], 15, 512) ?>'
            data-pendaftars='<?php echo json_encode($pendaftars ?? [], 15, 512) ?>'>
                ...
                <div x-data="{
                    ...
                    <button type="button" @click="toggle()"
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedPeriode): ?>
                            ...
                            <span><?php echo e($selectedPeriode->nama_periode); ?></span>
                        <?php else: ?>
                        ...
                        <svg class="w-4 h-4 flex-shrink-0 transition-transform duration-200 <?php echo e($selectedPeriode ? 'text-slate-400' : 'text-white/70'); ?>"
                        ...
                        <div class="overflow-y-auto max-h-64 py-1.5">
                            <template x-for="p in filtered" :key="p.id">
                                <a :href="p.url"
                                   ...
                                   :class="p.id == <?php echo e($selectedPeriodeId ?? 'null'); ?> ? 'bg-primary/10 text-primary' : 'text-slate-700'">
                                    <span class="w-1.5 h-1.5 rounded-full flex-shrink-0"
                                          :class="p.id == <?php echo e($selectedPeriodeId ?? 'null'); ?> ? 'bg-primary' : 'bg-slate-300'"></span>
                                    <span x-text="p.nama" class="flex-1"></span>
                                    <svg x-show="p.id == <?php echo e($selectedPeriodeId ?? 'null'); ?>" class="w-3.5 h-3.5 text-primary flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                                </a>
                            </template>
        ...
        <div class="flex items-center gap-2 mb-6">
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedPeriode->tanggal_mulai_ujian && $selectedPeriode->tanggal_selesai_ujian): ?>
                <span class="px-3 py-1 bg-primary/10 text-primary text-[13px] font-semibold border border-primary/20 rounded-lg">
                    <?php echo e(\Carbon\Carbon::parse($selectedPeriode->tanggal_mulai_ujian)->translatedFormat('d F Y')); ?> &ndash; <?php echo e(\Carbon\Carbon::parse($selectedPeriode->tanggal_selesai_ujian)->translatedFormat('d F Y')); ?>
        ...
        <div class="mb-8">
            <?php echo $__env->make('banksoal::alokasi-sesi._table', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        </div>
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($activePeriode): ?>
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($allocatedGroups->count() > 0): ?>
            ...
            <div class="mt-8 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="px-6 py-5 flex items-center justify-between border-b border-slate-200 bg-slate-50">
                    <h2 class="font-bold text-slate-800">Daftar Peserta <?php echo e($activePeriode->nama_periode); ?></h2>
                </div>
                <div class="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $allocatedGroups; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $jadwalId => $pesertas): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <?php
                        ...
                        <div class="bg-white border border-slate-200 rounded-[12px] overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.04)] flex flex-col">
                            <div class="px-5 py-4 border-b border-slate-200 bg-slate-50">
                                <h3 class="font-bold text-[15px] text-slate-900 mb-0 m-0">
                                    <?php echo e(is_numeric($jadwal->nama_sesi) ? 'Sesi ' . $jadwal->nama_sesi : ($jadwal->nama_sesi ?? 'Nama Sesi Kosong')); ?>: <?php echo e($tanggal); ?> pukul <?php echo e($mulai); ?>-<?php echo e($selesai); ?> WIB
                                </h3>
                            </div>
                            <div class="overflow-x-auto w-full">
                                <table class="w-full text-left text-[13px] border-collapse min-w-[380px]">
                                    ...
                                    <tbody class="bg-white">
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $pesertas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $peserta): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                                        <tr class="border-b border-[#F3F4F6] hover:bg-[#FAFAFA] transition-colors duration-150">
                                            ...
                                            <td class="px-4 py-3.5 font-semibold text-slate-800">
                                                <?php echo e($peserta->nim); ?>

                                            </td>
                                            <td class="px-4 py-3.5 font-medium text-slate-700">
                                                <?php echo e($peserta->nama_lengkap); ?>
                     ...
                     class="relative w-full max-w-2xl max-h-[85vh] flex flex-col bg-white rounded-2xl shadow-xl overflow-hidden">
                            ...
                            <div class="relative flex-1 overflow-y-auto bg-white p-6">
                                ...
                                <div x-show="drawerTab === 'unassigned'" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                                    <form action="<?php echo e(route('banksoal.pendaftaran.alokasi-sesi.store')); ?>" method="POST" id="formAssign">
                                        <?php echo csrf_field(); ?>
                                ...
                                <div x-show="drawerTab === 'assigned'" style="display: none;" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                                    <form action="<?php echo e(route('banksoal.pendaftaran.alokasi-sesi.remove')); ?>" method="POST" id="formRemove">
                                        <?php echo csrf_field(); ?>
        ...
        <div x-show="openModal" tabindex="-1" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" style="display: none;">
            ...
            <div x-show="openModal" 
                ...
                <div class="px-6 pb-6 overflow-y-auto w-full">
                    ...
                    <form action="<?php echo e(route('banksoal.periode.jadwal.store')); ?>" method="POST" id="formTambahSesi" class="space-y-4">
                        <?php echo csrf_field(); ?>
                        ...
                        <div>
                            ...
                            <input type="number" name="nama_sesi" value="<?php echo e(old('nama_sesi')); ?>" placeholder="1" min="1" required class="w-full px-4 py-2.5 bg-white rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-slate-800 placeholder-slate-400 transition-shadow <?php echo e($errors->has('nama_sesi') ? 'border-red-500 ring-red-500/20' : 'border border-slate-300'); ?>">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['nama_sesi'];
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        ...
                        <div>
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['tanggal_ujian'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="time" name="waktu_mulai" value="<?php echo e(old('waktu_mulai')); ?>" required class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-slate-800 transition-shadow">
                            </div>
                            <div>
                                <label class="block text-[13px] text-slate-700 mb-1.5 font-bold">Waktu Selesai</label>
                                <input type="time" name="waktu_selesai" value="<?php echo e(old('waktu_selesai')); ?>" required class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-slate-800 transition-shadow">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="kuota" value="<?php echo e(old('kuota')); ?>" placeholder="50" min="1" step="1" required class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-slate-800 placeholder-slate-400 transition-shadow">
                        </div>
                    </form>
                </div>
...
<?php endif; ?><?php /**PATH C:\WebsiteTekkom - Copy\Modules\BankSoal\resources\views\alokasi-sesi\index.blade.php ENDPATH**/ ?>