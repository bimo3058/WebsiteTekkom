...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
    ...
    <div class="w-full">
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($periodes->isEmpty()): ?>
                <div
                    ...
                    Belum ada periode. <a href="<?php echo e(route('banksoal.periode.setup')); ?>" class="underline font-bold">Buat
                        Periode Pertama</a>
                </div>
            <?php else: ?>
                    <?php
                            ...
                            <div class="overflow-y-auto max-h-64 py-1.5">
                                <template x-for="p in filtered" :key="p.id">
                                    <a :href="p.url"
                                        ...
                                        <svg x-show="p.id == <?php echo e($selectedPeriodeId ?? 'null'); ?>"
        ...
        <form method="GET" action="<?php echo e(route('banksoal.pendaftaran.index')); ?>" id="filter-form" class="hidden">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(request('periode_id')): ?>
                <input type="hidden" name="periode_id" value="<?php echo e(request('periode_id')); ?>">
            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            <input type="hidden" name="search" id="hidden-search" value="<?php echo e(request('search')); ?>">
            <input type="hidden" name="status" id="hidden-status" value="<?php echo e(request('status')); ?>">
            <input type="hidden" name="per_page" id="hidden-per-page" value="<?php echo e(request('per_page', 5)); ?>">
        </form>
        ...
        <div id="filter-container">
            <div style="background:#fff; border:1px solid var(--c-border); border-radius:14px; overflow:hidden; box-shadow:0 1px 3px rgba(0,0,0,.04); display:flex; flex-direction:column; margin-bottom: 2rem;">
                ...
                <div style="display:flex; align-items:center; justify-content:space-between; padding:14px 16px; border-bottom:1px solid var(--c-border); gap:10px; flex-wrap:wrap;">
                    ...
                    <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap; min-width:0;">
                        ...
                        <div style="position:relative; width:min(220px, calc(100vw - 200px)); min-width:120px;">
                            ...
                            <input type="text" value="<?php echo e(request('search')); ?>"
                                placeholder="Cari NIM atau nama..." <?php echo e(!request('periode_id') ? 'disabled' : ''); ?>
                                ...
                                style="width:100%; height:34px; padding:0 12px 0 34px; border:1px solid var(--c-border); border-radius:8px; font-size:12.5px; color:var(--c-fg); font-family:inherit; outline:none; transition:all .15s; box-sizing:border-box; background:<?php echo e(!request('periode_id') ? '#f8fafc' : '#fff'); ?>; <?php echo e(!request('periode_id') ? 'cursor:not-allowed; opacity:0.6;' : ''); ?>"
                                onfocus="if(<?php echo e(request('periode_id') ? 'true' : 'false'); ?>){ this.style.borderColor='var(--c-primary)'; this.style.boxShadow='0 0 0 3px rgba(94,83,244,0.08)' }"
                                onblur="if(<?php echo e(request('periode_id') ? 'true' : 'false'); ?>){ this.style.borderColor='var(--c-border)'; this.style.boxShadow='none' }">
                        </div>
                        ...
                        <div class="relative inline-block" x-data="{ open: false }">
                            <button type="button" @click="open = !open" @click.outside="open = false"
                                    ...
                                    <?php echo e(!request('periode_id') ? 'disabled' : ''); ?>>
                            ...
                            <div x-show="open" x-cloak
                                ...
                                <div class="p-1.5">
                                    ...
                                    <button type="button"
                                        onclick="document.getElementById('hidden-status').value = '<?php echo e($val); ?>'; document.getElementById('filter-form').submit();"
                ...
                <?php echo $__env->make('banksoal::pendaftaran.partials.table', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            </div>
        </div>

                <!-- Include Modal Components -->
                <?php echo $__env->make('banksoal::pendaftaran.partials.modal-manual', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                <?php echo $__env->make('banksoal::pendaftaran.partials.modal-detail', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            </div>

            
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($errors->hasBag('pendaftar') && $errors->getBag('pendaftar')->any()): ?>
                <script>
...
<?php endif; ?>
<?php /**PATH C:\WebsiteTekkom - Copy\Modules\BankSoal\resources\views\pendaftaran\index.blade.php ENDPATH**/ ?>