...
<aside data-mobile-sidebar :class="sidebarOpen ? 'w-[240px]' : 'w-[64px]'"
    ...
    <div class="flex-1 overflow-y-auto overflow-x-hidden p-[6px_10px_10px] flex flex-col gap-[1px] scrollbar-thin scrollbar-thumb-[#DFE1E7] scrollbar-track-transparent">
        ...
        <?php $isDashboard = request()->routeIs('banksoal.dashboard') && auth()->user()->hasRole('gpm'); ?>
        ...
        ?>
        <div x-data="{ openAcc: <?php echo e($isManajemenModulActive ? 'true' : 'false'); ?> }" class="flex flex-col mt-[1px]">
            <button @click="if(!sidebarOpen){ sidebarOpen=true; openAcc=true; } else { openAcc=!openAcc }" class="relative w-full flex items-center gap-[9px] p-[7px_10px_7px_14px] rounded-lg text-[13px] transition-colors <?php echo e($isManajemenModulActive ? 'bg-[#F6F8FA] text-[#0D0D12] font-semibold' : 'text-[#353849] font-medium hover:bg-[#F6F8FA]'); ?>" :class="!sidebarOpen ? 'justify-center p-[7px_0]' : ''">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isManajemenModulActive): ?>
                    <span class="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-[20px] bg-[#0B266E] rounded-r-[3px]" x-show="sidebarOpen"></span>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                <svg class="w-4 h-4 flex-shrink-0 <?php echo e($isManajemenModulActive ? 'text-[#0B266E]' : 'text-[#666D80]'); ?>" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="<?php echo e($iconManajemen); ?>"/></svg>
            ...
            <div x-show="openAcc && sidebarOpen" x-collapse x-cloak>
                <div class="flex flex-col gap-[2px] pl-[34px] pr-[10px] py-[2px] mt-[1px]">
                    <a href="<?php echo e(route('banksoal.soal.gpm.parameter.index')); ?>" class="block p-[7px_10px] rounded-lg text-[13px] transition-colors <?php echo e($isKontrolUmum ? 'bg-[#F6F8FA] text-[#0D0D12] font-semibold' : 'text-[#353849] hover:bg-[#F6F8FA]'); ?>">Kontrol Umum</a>
                    <a href="<?php echo e(route('banksoal.rps.gpm.validasi-rps')); ?>" class="block p-[7px_10px] rounded-lg text-[13px] transition-colors <?php echo e($isValidasiRps ? 'bg-[#F6F8FA] text-[#0D0D12] font-semibold' : 'text-[#353849] hover:bg-[#F6F8FA]'); ?>">Validasi RPS</a>
                    <a href="<?php echo e(route('banksoal.soal.gpm.validasi-bank-soal')); ?>" class="block p-[7px_10px] rounded-lg text-[13px] transition-colors <?php echo e($isValidasiSoal ? 'bg-[#F6F8FA] text-[#0D0D12] font-semibold' : 'text-[#353849] hover:bg-[#F6F8FA]'); ?>">Validasi Soal</a>
                </div>
            </div>
        </div>
        ...
        ?>
        <div x-data="{ openAcc: <?php echo e($isRiwayat ? 'true' : 'false'); ?> }" class="flex flex-col mt-[1px]">
            <button @click="if(!sidebarOpen){ sidebarOpen=true; openAcc=true; } else { openAcc=!openAcc }" class="relative w-full flex items-center gap-[9px] p-[7px_10px_7px_14px] rounded-lg text-[13px] transition-colors <?php echo e($isRiwayat ? 'bg-[#F6F8FA] text-[#0D0D12] font-semibold' : 'text-[#353849] font-medium hover:bg-[#F6F8FA]'); ?>" :class="!sidebarOpen ? 'justify-center p-[7px_0]' : ''">
            ...
            <div x-show="openAcc && sidebarOpen" x-collapse x-cloak>
                <div class="flex flex-col gap-[2px] pl-[34px] pr-[10px] py-[2px] mt-[1px]">
                    <a href="<?php echo e(route('banksoal.rps.gpm.riwayat-validasi.rps')); ?>" class="block p-[7px_10px] rounded-lg text-[13px] transition-colors <?php echo e($isRiwayatRps ? 'bg-[#F6F8FA] text-[#0D0D12] font-semibold' : 'text-[#353849] hover:bg-[#F6F8FA]'); ?>">Riwayat Validasi RPS</a>
                    <a href="<?php echo e(route('banksoal.soal.gpm.riwayat-validasi.bank-soal')); ?>" class="block p-[7px_10px] rounded-lg text-[13px] transition-colors <?php echo e($isRiwayatSoal ? 'bg-[#F6F8FA] text-[#0D0D12] font-semibold' : 'text-[#353849] hover:bg-[#F6F8FA]'); ?>">Riwayat Validasi Bank Soal</a>
                </div>
            </div>
        </div>

        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->user()->hasRole('dosen')): ?>
            <div x-show="sidebarOpen" class="text-[10px] font-semibold text-[#808897] tracking-[0.06em] uppercase p-[12px_10px_5px] whitespace-nowrap mt-1">Dosen Pengampu</div>
            <a href="<?php echo e(route('banksoal.switch-role', 'dosen')); ?>" class="relative flex items-center gap-[9px] p-[7px_10px_7px_14px] rounded-lg text-[13px] mt-[1px] transition-colors text-[#353849] font-medium hover:bg-[#F6F8FA]" :class="!sidebarOpen ? 'justify-center p-[7px_0]' : ''">
                ...
                <span x-show="sidebarOpen" class="flex-1 whitespace-nowrap overflow-hidden text-ellipsis leading-tight">Akses Dosen</span>
            </a>
        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
    </div>
...
<?php /**PATH C:\WebsiteTekkom - Copy\Modules\BankSoal\resources\views\components\ui\sidebar-gpm.blade.php ENDPATH**/ ?>