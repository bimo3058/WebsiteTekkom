...
<div style="background:#fff; border:1px solid var(--c-border); border-radius:14px; overflow:hidden; box-shadow:0px 1px 2px rgba(228,229,231,0.24); transition:border-color .15s, box-shadow .15s; display:flex; flex-direction:column;"
    ...
    <div style="padding:16px;">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:14px;">
            <div style="width:38px; height:38px; border-radius:10px; background:<?php echo e($col['bg']); ?>; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                <svg width="18" height="18" fill="none" stroke="<?php echo e($col['color']); ?>" viewBox="0 0 24 24" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div style="display:flex; flex-direction:column; align-items:flex-end; gap:4px;">
                ...
                <form action="<?php echo e(route('superadmin.modules.toggle', $module->slug)); ?>" method="POST">
                    <?php echo csrf_field(); ?>
                    <button type="submit"
                            title="<?php echo e($isOn ? 'Nonaktifkan' : 'Aktifkan'); ?>"
                            style="position:relative; width:32px; height:18px; border-radius:9999px; border:none; cursor:pointer; background:<?php echo e($isOn ? 'var(--c-primary)' : 'var(--c-border)'); ?>; transition:background .2s; padding:0; flex-shrink:0;">
                        <div style="position:absolute; top:2px; left:2px; width:14px; height:14px; border-radius:50%; background:#fff; box-shadow:0 1px 2px rgba(0,0,0,0.15); transition:transform .2s; transform:<?php echo e($isOn ? 'translateX(14px)' : 'none'); ?>;"></div>
                    </button>
                </form>
                <span style="font-size:9px; font-weight:700; text-transform:uppercase; letter-spacing:.06em; color:<?php echo e($isOn ? 'var(--c-primary)' : 'var(--c-fg-muted)'); ?>;">
                    <?php echo e($isOn ? 'Active' : 'Inactive'); ?>
        ...
        <h3 style="font-size:13px; font-weight:700; color:var(--c-fg); margin-bottom:4px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"><?php echo e($module->name); ?></h3>
        <p style="font-size:11px; color:var(--c-fg-muted); line-height:1.5; margin-bottom:14px; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; min-height:32px;">
            <?php echo e($module->description ?? 'Fungsional modul ' . $module->name . '.'); ?>
        ...
        <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:6px; margin-bottom:14px;">
            ...
            <div style="background:var(--c-bg); border:1px solid var(--c-border); border-radius:8px; padding:7px 8px;">
                <p style="font-size:8px; font-weight:700; text-transform:uppercase; letter-spacing:.08em; color:var(--c-fg-placeholder); margin-bottom:2px;"><?php echo e($label); ?></p>
                <p style="font-size:11px; font-weight:700; color:<?php echo e(($label === 'Debug' && $module->setting('debug_mode')) ? 'var(--c-warning)' : 'var(--c-fg-sec)'); ?>;"><?php echo e($val); ?></p>
            </div>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
        </div>

        
        <button onclick="openModal('modal-<?php echo e($module->slug); ?>')"
...
</div><?php /**PATH C:\WebsiteTekkom - Copy\resources\views\superadmin\modules\_card.blade.php ENDPATH**/ ?>