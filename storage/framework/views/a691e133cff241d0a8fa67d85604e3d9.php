...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'eoffice::components.layouts.koordinator','data' => ['title' => 'Template Dokumen']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Template Dokumen']); ?>
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
    ...
    <div x-data="{
        ...
        deleteId: null,
        isEditing: false,
        formAction: '<?php echo e(route('eoffice.kp.koordinator.template.store')); ?>',
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(session('error') || session('warning')): ?>
            <div
                class="mb-6 p-4 rounded-xl border <?php echo e(session('error') ? 'bg-red-50 border-red-200 text-red-700' : 'bg-amber-50 border-amber-200 text-amber-700'); ?> flex items-start gap-3">
                <svg class="w-5 h-5 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                </svg>
                <div>
                    <h4 class="font-bold"><?php echo e(session('error') ? 'Error' : 'Peringatan'); ?></h4>
                    <p class="text-sm mt-1"><?php echo e(session('error') ?? session('warning')); ?></p>
                </div>
            </div>
        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        ...
                        <div class="w-full flex flex-col justify-end h-full space-y-4 md:w-64">
                            <div class="flex gap-2 w-full mt-7">
                                <button type="button" x-show="isEditing"
                                    @click="isEditing = false; formAction = '<?php echo e(route('eoffice.kp.koordinator.template.store')); ?>'; formData = { title: '', phase: 'pra_kp', fileName: '' }"
        ...
        <!-- Template List -->
        <div class="space-y-4">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $templates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                <div
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-3 mb-1">
                            <span
                                class="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide <?php echo e($item->phase == 'pra_kp' ? 'bg-amber-50 text-amber-700 border-amber-100' : ($item->phase == 'saat_kp' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-emerald-50 text-emerald-700 border-emerald-100')); ?> border uppercase">
                                <?php echo e(str_replace('_', ' ', $item->phase)); ?>

                            </span>
                        </div>
                        <h3 class="text-lg font-bold text-slate-900 mb-1 truncate"><?php echo e($item->title); ?></h3>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($item->file_path): ?>
                            <a href="<?php echo e(Storage::disk('public')->url($item->file_path)); ?>" target="_blank"
                                ...
                                <?php echo e($item->file_name ?? basename($item->file_path)); ?>
                    ...
                    <div class="flex flex-row items-center gap-2 shrink-0">
                        <button type="button"
                            @click='formData = { title: <?php echo json_encode($item->title, 15, 512) ?>, phase: <?php echo json_encode($item->phase, 15, 512) ?>, fileName: <?php echo json_encode($item->file_name ?? basename($item->file_path ?? ""), 15, 512) ?> }; formAction = `/eoffice/kp/koordinator/template/<?php echo e($item->id); ?>`; isEditing = true; document.getElementById("form-section").scrollIntoView({ behavior: "smooth" });'
...
<?php endif; ?><?php /**PATH C:\WebsiteTekkom - Copy\Modules\EOffice\resources\views\koordinator\template.blade.php ENDPATH**/ ?>