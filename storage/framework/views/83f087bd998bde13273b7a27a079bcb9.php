<div style="background:#fff; border:1px solid var(--c-border); border-radius:14px; overflow:hidden; box-shadow:0 1px 3px rgba(0,0,0,.04); display:flex; flex-direction:column;">
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; padding:14px 16px; border-bottom:1px solid var(--c-border); gap:10px; flex-wrap:wrap;">
        ...
        <?php echo $__env->make('banksoal::alokasi-sesi._search_filter', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    </div>
        ...
        <table style="width:100%; border-collapse:collapse; min-width:850px;">
            <thead>
                <tr style="border-bottom:1px solid var(--c-border); background:#FAFAFA;">
                    ...
                    <th style="padding:11px 16px; text-align:left; font-size:11px; font-weight:600; color:var(--c-fg-muted); white-space:nowrap; width: 200px;">Alokasi & Kuota</th>
            ...
            <tbody>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $jadwals; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $jadwal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                    <?php
                    ...
                    ?>
                    <tr style="border-bottom:1px solid #F3F4F6; transition:background .12s; cursor:pointer;" onmouseover="this.style.background='#FAFAFA'" onmouseout="this.style.background='transparent'" @click="openJadwalDrawer(<?php echo e($jadwal->id); ?>)">
                        <td style="padding:14px 16px; font-size:13px; font-weight:400; color:var(--c-fg-muted);"><?php echo e($rowNo); ?></td>
                        
                        <td style="padding:14px 16px;">
                            <p style="font-size:13px; font-weight:600; color:var(--c-fg); margin:0; transition:color .12s;" onmouseover="this.style.color='var(--c-primary)'" onmouseout="this.style.color='var(--c-fg)'">
                        ...
                        <td style="padding:14px 16px;">
                            <p style="font-size:13px; font-weight:500; color:var(--c-fg); margin:0;">
                                <?php echo e($jadwal->tanggal_ujian ? \Carbon\Carbon::parse($jadwal->tanggal_ujian)->translatedFormat('d F Y') : '-'); ?>
                        ...
                        <td style="padding:14px 16px;">
                            <p style="font-size:13px; font-weight:500; color:var(--c-fg); margin:0;">
                                <?php echo e(\Carbon\Carbon::parse($jadwal->waktu_mulai)->format('H:i')); ?> - <?php echo e(\Carbon\Carbon::parse($jadwal->waktu_selesai)->format('H:i')); ?> WIB
                            </p>
                        </td>

                        <td style="padding:14px 16px;">
                            <div style="display:flex; align-items:center; gap:10px;">
                                <div style="flex:1; background:#F1F5F9; border-radius:9999px; height:6px; overflow:hidden;">
                                    <div style="height:100%; border-radius:9999px; background:<?php echo e($isFull ? '#EF4444' : 'var(--c-primary)'); ?>; width: <?php echo e(min($percent, 100)); ?>%; transition:width 0.5s ease-out;"></div>
                                </div>
                                <span style="font-size:11px; font-weight:700; color:<?php echo e($isFull ? '#DC2626' : 'var(--c-fg-muted)'); ?>;"><?php echo e($jadwal->terisi); ?> / <?php echo e($jadwal->kuota); ?></span>
                            </div>
                        </td>

                        <td style="padding:14px 16px; text-align:right;">
                            <div style="position:relative; display:inline-block; text-align:left;" x-data="{ actionOpen: false }">
                                <button type="button" @click.stop="actionOpen = !actionOpen" @click.outside="actionOpen = false"
                                ...
                                <div x-show="actionOpen"
                                    ...
                                    <div style="padding:5px;">
                                        ...
                                        <button type="button" @click.stop="openJadwalDrawer(<?php echo e($jadwal->id); ?>); actionOpen = false" style="width:100%; display:flex; align-items:center; gap:8px; padding:7px 10px; border:none; border-radius:6px; background:none; font-size:11px; font-weight:500; color:var(--c-primary); cursor:pointer; font-family:inherit; text-align:left; transition:background .12s;" onmouseover="this.style.background='rgba(94,83,244,0.06)'" onmouseout="this.style.background='none'">
                                            <svg width="13" height="13" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8zm14 10v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>
                                            Kelola Peserta
                                        </button>
                    ...
                    <tr>
                        <td colspan="6" style="padding:60px 24px; text-align:center;">
                            <div style="display:flex; flex-direction:column; align-items:center; gap:8px;">
                                ...
                                <p style="font-size:12px; font-weight:600; color:var(--c-fg-muted); text-transform:uppercase; letter-spacing:0.06em;">Belum Ada Sesi Ujian</p>
                                <p style="font-size:11px; color:var(--c-fg-placeholder);">
                                    <?php echo e($selectedPeriode ? 'Buat sesi pertama untuk mulai mengalokasikan jadwal peserta.' : 'Pilih periode ujian di atas untuk menampilkan sesi ujian.'); ?>
    ...
    <?php echo $__env->make('banksoal::alokasi-sesi._pagination', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
</div>
<?php /**PATH C:\WebsiteTekkom - Copy\Modules\BankSoal\resources\views\alokasi-sesi\_table.blade.php ENDPATH**/ ?>