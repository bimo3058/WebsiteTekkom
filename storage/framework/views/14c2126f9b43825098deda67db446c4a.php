...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
    ...
    <span class="mx-2 text-slate-300">/</span>
    <span class="text-slate-800 font-semibold">Edit Pemetaan MK ke CPL</span>
    ...
    <div class="page-header">
        <div>
            <h1>Edit Pemetaan MK ke CPL</h1>
            <p>Mata Kuliah sudah terkunci. Ubah centang CPL yang dipetakan ke MK ini.</p>
        </div>
        ...
        <div class="panel">
            ...
            <div class="panel-search">
                <div class="srch">
                    ...
                    <input type="text" id="rightSearch" placeholder="Cari CPL..." oninput="onSearch(this.value)">
                </div>
            </div>
            <div class="sa-row">
            ...
            <div class="r-empty" id="rightEmpty" style="display:none;">Tidak ada data CPL</div>
    ...
    <script>
    const CSRF='<?php echo e(csrf_token()); ?>',BASE_API='<?php echo e(url("/bank-soal/admin/api")); ?>',BACK_URL='<?php echo e(route("banksoal.admin.kontrol-umum.pemetaan")); ?>';
    const MK_ID=<?php echo e($mk->id); ?>,RS=10;
    const s={pool:[],sel:new Set(<?php echo e(json_encode($selectedIds)); ?>),q:'',p:1};
    document.addEventListener('DOMContentLoaded',async()=>{
        window.showLoader();
        try{const r=await fetch(BASE_API+'/pemetaan/options',{headers:{'Accept':'application/json','X-CSRF-TOKEN':CSRF}});const d=await r.json();if(d.success)s.pool=d.data.cpl.sort((a,b)=>a.kode.localeCompare(b.kode,undefined,{numeric:true}));}catch(e){console.error(e);}
        window.hideLoader();render();
    });
    function onSearch(v){s.q=v;s.p=1;render();}
    function filt(){return s.pool.filter(c=>c.kode.toLowerCase().includes(s.q.toLowerCase()));}
    function render(){
    ...
    async function submitMapping(){
        ...
        window.showLoader();
        try{const r=await fetch(BASE_API+'/pemetaan/mk-cpl/sync',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json','X-CSRF-TOKEN':CSRF},body:JSON.stringify({mk_id:MK_ID,cpl_ids:[...s.sel]})});const d=await r.json();
        if(r.ok&&d.success){window.dispatchEvent(new CustomEvent('notify', { detail: { type: 'success', message: d.message || 'Pemetaan berhasil diperbarui.' } }));setTimeout(() => { window.location.href=BACK_URL; }, 1500);}
        else{window.dispatchEvent(new CustomEvent('notify', { detail: { type: 'error', message: d.message || 'Terjadi kesalahan.' } }));btn.disabled=false;btn.innerHTML='Simpan Perubahan';}
        }catch(e){window.dispatchEvent(new CustomEvent('notify', { detail: { type: 'error', message: e.message || 'Terjadi kesalahan jaringan.' } }));btn.disabled=false;btn.innerHTML='Simpan Perubahan';}
...
<?php endif; ?>
<?php /**PATH C:\WebsiteTekkom - Copy\Modules\BankSoal\resources\views\pages\admin\kontrol-umum\pemetaan-mk-cpl-edit.blade.php ENDPATH**/ ?>