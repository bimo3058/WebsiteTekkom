...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'eoffice::components.layouts.dosen','data' => ['title' => 'Detail Bimbingan']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Detail Bimbingan']); ?>
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>

    <?php $__env->startSection('breadcrumbs'); ?>
        <a href="<?php echo e(route('eoffice.kp.dosen.dashboard')); ?>" class="text-slate-400 hover:text-slate-700 transition-colors mr-2">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="px-6 py-6 border-b border-slate-200 bg-slate-50/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div class="flex items-center gap-4">
                    <div class="h-14 w-14 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-bold text-xl border-2 border-white shadow-sm">
                        <?php echo e(substr($kp->nim ?? 'M', 0, 2)); ?>

                    </div>
                    <div>
                        <h2 class="text-xl font-bold text-slate-900">Mahasiswa (<?php echo e($kp->nim ?? 'Belum Diisi'); ?>)</h2>
                        <div class="flex items-center gap-2 mt-1">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($kp->status_kp == 'pending' || !$kp->status_kp): ?>
                            ...
                            <?php elseif($kp->status_kp == 'active'): ?>
                                <span class="px-2.5 py-1 rounded-md text-[11px] font-bold bg-blue-100 text-blue-800 uppercase tracking-wider">Fase: Saat KP</span>
                            <?php else: ?>
                            ...
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            <span class="text-xs text-slate-500">Diajukan pada <?php echo e($kp->created_at->format('d M Y')); ?></span>
                        </div>
                    </div>
                </div>
                ...
                <?php elseif($kp->status_kp == 'active'): ?>
                    <a href="<?php echo e(route('eoffice.kp.dosen.bimbingan.penilaian', $kp->id)); ?>" class="w-full md:w-auto inline-flex items-center justify-center px-4 py-2 text-sm font-semibold rounded-lg shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 transition-colors">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        Beri Nilai KP
                    </a>
                <?php elseif($kp->status_kp == 'completed'): ?>
                    <a href="<?php echo e(route('eoffice.kp.dosen.bimbingan.penilaian', $kp->id)); ?>" class="w-full md:w-auto inline-flex items-center justify-center px-4 py-2 text-sm font-semibold rounded-lg shadow-sm text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 transition-colors">
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
                    <div>
                        ...
                        <p class="mt-1 text-base font-semibold text-slate-900"><?php echo e($kp->judul_kp ?? '-'); ?></p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-slate-500">Instansi / Perusahaan Tujuan</p>
                        <p class="mt-1 text-base font-semibold text-slate-900"><?php echo e($kp->instansi_kp ?? '-'); ?></p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-slate-500">Tanggal Mulai</p>
                        <p class="mt-1 text-base font-medium text-slate-900"><?php echo e($kp->tanggal_mulai ? \Carbon\Carbon::parse($kp->tanggal_mulai)->format('d F Y') : '-'); ?></p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-slate-500">Tanggal Selesai</p>
                        <p class="mt-1 text-base font-medium text-slate-900"><?php echo e($kp->tanggal_selesai ? \Carbon\Carbon::parse($kp->tanggal_selesai)->format('d F Y') : '-'); ?></p>
                    </div>
                </div>
                    ...
                    <?php else: ?>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(session('error')): ?>
                            <div class="mb-4 bg-red-50 border-l-4 border-red-500 p-4 rounded shadow-sm">
                                <p class="text-red-700 font-medium text-sm"><?php echo e(session('error')); ?></p>
                            </div>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                ...
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $dokumens; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dok): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                                    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 border border-slate-200 rounded-xl bg-white hover:shadow-sm transition-shadow">
                                        <div class="flex items-center gap-4 mb-4 sm:mb-0">
                                            <div class="h-12 w-12 bg-indigo-50 rounded-lg flex items-center justify-center text-indigo-500 border border-indigo-100">
                                                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                            </div>
                                            <div>
                                                <p class="text-base font-semibold text-slate-900"><?php echo e($dok->jenis_dokumen); ?> KP</p>
                                                <div class="flex items-center gap-2 mt-1">
                                                    <a href="<?php echo e($dok->file_url); ?>" class="text-xs text-blue-600 hover:text-blue-800 underline font-medium">Lihat File (<?php echo e(basename($dok->file_path)); ?>)</a>
                                                    <span class="text-slate-300">•</span>
                                                    <span class="text-xs text-slate-500"><?php echo e(\Carbon\Carbon::parse($dok->tanggal_upload)->format('d M Y')); ?></span>
                                                </div>
                                            </div>
                                        </div>
                                        
                                        <div class="w-full sm:w-auto flex items-center gap-2">
                                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($dok->status_validasi == 'pending'): ?>
                                                <!-- Action Buttons for Dosen -->
                                                <form action="<?php echo e(route('eoffice.kp.dosen.bimbingan.dokumen.reject', [$kp->id, $dok->id])); ?>" method="POST" class="w-1/2 sm:w-auto">
                                                    <?php echo csrf_field(); ?>
                                                ...
                                                <form action="<?php echo e(route('eoffice.kp.dosen.bimbingan.dokumen.approve', [$kp->id, $dok->id])); ?>" method="POST" class="w-1/2 sm:w-auto">
                                                    <?php echo csrf_field(); ?>
                                            ...
                                            <?php elseif($dok->status_validasi == 'approved'): ?>
                                            ...
                                            <?php elseif($dok->status_validasi == 'rejected'): ?>
...
<?php endif; ?>
<?php /**PATH C:\WebsiteTekkom - Copy\Modules\EOffice\resources\views\dosen\show.blade.php ENDPATH**/ ?>