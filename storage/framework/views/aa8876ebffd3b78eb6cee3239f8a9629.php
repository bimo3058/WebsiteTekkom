...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'eoffice::components.layouts.koordinator','data' => ['title' => 'Edit Periode']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Edit Periode']); ?>
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>

    <?php $__env->startSection('breadcrumbs'); ?>
        <a href="<?php echo e(route('eoffice.kp.koordinator.periode')); ?>"
    ...
    <div x-data="periodeWizard()" x-init="initDates()" class="w-full pb-20">
        ...
        <div class="mb-6">
            <a href="<?php echo e(route('eoffice.kp.koordinator.periode')); ?>" style="
        ...
        <div class="flex justify-between items-center mb-8">
            ...
            <button type="button" x-show="step < 4" @click="nextStep()" :disabled="step === 3 && totalBobot() !== 100"
            ...
            <button type="submit" form="periodeWizardForm" x-show="step === 4" :disabled="totalBobot() !== 100"
                class="flex items-center justify-center px-4 py-2 border border-transparent text-white rounded-xl transition-colors shadow-sm text-sm font-semibold"
        ...
        <form id="periodeWizardForm" x-ref="form"
            action="<?php echo e(route('eoffice.kp.koordinator.periode.update', $periode->id)); ?>" method="POST">
            <?php echo csrf_field(); ?>
    ...
    <script>
        function periodeWizard() {
            return {
                ...
                masterRubriks: <?php echo json_encode($masterRubriks, 15, 512) ?>,
                formData: {
                    semester: '<?php echo e(old('semester', $periode->semester)); ?>',
                    tahun_ajaran: '<?php echo e(old('tahun_ajaran', $periode->tahun_ajaran)); ?>',
                    kelas_dibuka: '<?php echo e(is_array(old('kelas_dibuka', $periode->kelas_dibuka)) ? implode(', ', old('kelas_dibuka', $periode->kelas_dibuka)) : old('kelas_dibuka', $periode->kelas_dibuka)); ?>',
                    tanggal_buka: '<?php echo e(old('tanggal_buka', $periode->tanggal_buka)); ?>',
                    tanggal_tutup: '<?php echo e(old('tanggal_tutup', $periode->tanggal_tutup)); ?>',
                    is_active: <?php echo e(old('is_active', $periode->is_active) ? 'true' : 'false'); ?>,
                    pra_kp_mulai: '<?php echo e(old('pra_kp_mulai', $periode->pra_kp_mulai?->format('Y-m-d'))); ?>',
                    pra_kp_akhir: '<?php echo e(old('pra_kp_akhir', $periode->pra_kp_akhir?->format('Y-m-d'))); ?>',
                    pra_kp_pengingat: '<?php echo e(old('pra_kp_pengingat', $periode->pra_kp_pengingat?->format('Y-m-d'))); ?>',
                    saat_kp_mulai: '<?php echo e(old('saat_kp_mulai', $periode->saat_kp_mulai?->format('Y-m-d'))); ?>',
                    saat_kp_akhir: '<?php echo e(old('saat_kp_akhir', $periode->saat_kp_akhir?->format('Y-m-d'))); ?>',
                    saat_kp_pengingat: '<?php echo e(old('saat_kp_pengingat', $periode->saat_kp_pengingat?->format('Y-m-d'))); ?>',
                    pasca_kp_mulai: '<?php echo e(old('pasca_kp_mulai', $periode->pasca_kp_mulai?->format('Y-m-d'))); ?>',
                    pasca_kp_akhir: '<?php echo e(old('pasca_kp_akhir', $periode->pasca_kp_akhir?->format('Y-m-d'))); ?>',
                    pasca_kp_pengingat: '<?php echo e(old('pasca_kp_pengingat', $periode->pasca_kp_pengingat?->format('Y-m-d'))); ?>',
                    komponen_penilaian: <?php echo json_encode(collect(old('komponen_penilaian', $periode->komponenNilai))->values()->toArray()); ?>

                },
                allPeriodes: <?php echo json_encode($allPeriodes->map(function ($p) {
    return [
        'id' => $p->id,
        'semester' => $p->semester,
            ...
            return [
                ...
                toggleRubrik(mr, checked) {
                    if (checked) {
                        ...
                        // Tolak filter jika id kosong (dihapus)
...
<?php endif; ?><?php /**PATH C:\WebsiteTekkom - Copy\Modules\EOffice\resources\views\koordinator\periode\edit.blade.php ENDPATH**/ ?>