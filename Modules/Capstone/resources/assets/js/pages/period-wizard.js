...
export function periodWizard(id=null){return {
    ...
    validate(step){this.errors={};const fail=(key,message)=>{this.errors[key]=[message];};
        ...
        if(step===1){if(!this.hasTemplates)fail('assessments','Silakan lengkapi setup evaluasi terlebih dahulu.');for(const type of types){const ids=this.assessments[type];if(ids.length && Math.abs(this.weight(ids)-100)>.01)fail('assessments.'+type,type+': total bobot harus 100%.');}if(this.peerIds.length && Math.abs(this.weight(this.peerIds,true)-100)>.01)fail('peer_ids','Peer Review: total bobot harus 100%.');}
