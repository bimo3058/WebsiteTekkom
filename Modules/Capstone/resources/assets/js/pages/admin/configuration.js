import {basePage,api,rows,unwrap,query,dialog} from './shared.js';
...
export function assessmentConfig(peer=false,edit=false){return {...basePage(),peer,edit,type:peer?'PEER_REVIEW':(context.params?.id || 'SEMPRO'),templates:[],selectedIds:[],copyFrom:'',version:0,
    ...
    async load(){const version=++this.version;this.loading=true;this.error='';try{if(!this.periodId){this.items=[];return;}
        ...
        else {const types=this.peer?['PEER_REVIEW']:evaluationTypes;const configs=await Promise.all(types.map(async type=>{const data=unwrap(await api(this.endpoint()+query(this.peer?{}:{type})));return {id:type,name:type.replaceAll('_',' '),components:data.selected_indicators || data.selected_components || []};}));if(version===this.version)this.items=configs;}
    }catch(e){if(version===this.version)this.error=e.message;}finally{if(version===this.version)this.loading=false;}},
...
export function gradeConfig(){return {...basePage(),weights:{pdc1:{},pdc2:{},ta:{}},version:0,
