...
export function taRegistrationsAdmin(){return mergePage(basePage(),{status:'PENDING',decision:'approve',reason:'',pagination:{current_page:1,last_page:1,total:0},
    ...
    review(item,decision){this.selected=item;this.decision=decision;this.reason='';this.errors={};dialog('sidang-review').showModal();},
    async submit(){if(!this.selected||this.saving||(this.decision==='reject'&&!this.reason.trim()))return;const ok=await this.run(()=>api('/admin/ta-registrations/'+this.selected.id+'/'+this.decision,{method:'PUT',body:this.decision==='reject'?{rejection_reason:this.reason.trim()}:{}}),this.decision==='approve'?'Sidang TA request approved':'Sidang TA request rejected');if(ok)dialog('sidang-review').close();},
