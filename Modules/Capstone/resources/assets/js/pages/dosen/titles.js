...
export function lecturerTitles(){
    return workspace({items:[],specializations,filterSpecs:[],sortKey:'title',sortDirection:1,editing:null,deleting:null,
        ...
        get filtered(){return this.items.filter(t=>this.matches(t)&&(!this.filterSpecs.length||this.filterSpecs.some(s=>(t.specializations||[]).includes(s)))).sort((a,b)=>String(a[this.sortKey]??'').localeCompare(String(b[this.sortKey]??''),'en',{numeric:true})*this.sortDirection);},
