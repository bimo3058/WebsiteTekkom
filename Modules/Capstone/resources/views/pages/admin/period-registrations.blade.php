...
@section('content')
<div x-data="{tab:'join'}" class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-xl font-bold tracking-tight text-slate-900">Approval</h1>
            <p class="text-sm text-slate-500">Review student requests: period join requests and sidang TA registrations.</p>
        </div>
    </div>

    <div class="flex gap-1 rounded-xl border border-slate-200 bg-white p-1 shadow-sm w-fit" role="tablist" aria-label="Approval types">
        <button type="button" role="tab" :aria-selected="tab==='join'" @click="tab='join'" class="rounded-lg px-4 py-1.5 text-[13px] font-semibold" :class="tab==='join'?'bg-slate-900 text-white':'text-slate-500 hover:bg-slate-50'">Join Requests</button>
        <button type="button" role="tab" :aria-selected="tab==='sidang'" @click="tab='sidang'" class="rounded-lg px-4 py-1.5 text-[13px] font-semibold" :class="tab==='sidang'?'bg-slate-900 text-white':'text-slate-500 hover:bg-slate-50'">Sidang TA</button>
    </div>

    <div x-show="tab==='join'" x-data="adminPeriodRegistrations" class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h2 class="text-base font-semibold text-slate-900">Period Join Requests</h2>
    ...
    <div x-show="tab==='sidang'" x-data="adminTaRegistrations" class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h2 class="text-base font-semibold text-slate-900">Sidang TA Registrations</h2>
            <p class="text-sm text-slate-500">Review student requests to enter the TA document-upload phase. Approved students can upload the required TA documents.</p>
        </div>
        <x-capstone::button variant="outline" @click="load()" ::disabled="loading"><x-capstone::icon name="RefreshCw" size="15" />Refresh</x-capstone::button>
    </div>
    ...
    <div x-show="!loading && !error" x-cloak class="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div class="flex flex-wrap items-center gap-2.5 border-b border-slate-100 p-3">
            <label class="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-1.5 text-[13px] text-slate-500">
                <x-capstone::icon name="Search" size="15" />
                <input x-model.debounce.300ms="search" @input="filter()" type="search" placeholder="Cari nama / NIM..." class="w-44 bg-transparent outline-none placeholder:text-slate-400" aria-label="Cari pendaftaran sidang">
            </label>
            <select x-model="status" @change="filter()" class="rounded-lg border border-slate-200 bg-transparent px-2.5 py-1.5 text-[13px] text-slate-600 outline-none" aria-label="Filter status">
                <option value="PENDING">Pending</option>
                <option value="APPROVED">Approved</option>
            ...
            <table class="w-full min-w-[860px] text-left text-[13px]">
                <thead class="border-b border-slate-100 bg-slate-50/60 text-xs text-slate-500">
                    <tr>
                        <th class="px-4 py-2.5 font-medium">Mahasiswa</th>
                        <th class="px-4 py-2.5 font-medium">NIM</th>
                        <th class="px-4 py-2.5 font-medium">Grup</th>
                        <th class="px-4 py-2.5 font-medium">Periode</th>
                        <th class="px-4 py-2.5 font-medium">Status</th>
                        <th class="px-4 py-2.5 font-medium">Diminta</th>
                        <th class="px-4 py-2.5 text-right font-medium">Aksi</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    <template x-for="item in items" :key="item.id">
                        <tr class="align-top hover:bg-slate-50/60">
                            <td class="px-4 py-3 font-medium text-slate-800" x-text="studentName(item)"></td>
                            <td class="whitespace-nowrap px-4 py-3 text-slate-600" x-text="studentNim(item)"></td>
                            <td class="whitespace-nowrap px-4 py-3 text-slate-700" x-text="groupName(item)"></td>
                            <td class="whitespace-nowrap px-4 py-3 text-slate-700" x-text="item.period?.name || ('Period #'+item.period_id)"></td>
                            <td class="whitespace-nowrap px-4 py-3"><span class="rounded-full px-2 py-0.5 text-[11px] font-semibold" :class="statusClass(item.status)" x-text="item.status"></span><p x-show="item.status==='REJECTED' && item.rejection_reason" class="mt-1 max-w-64 whitespace-normal text-xs text-slate-500" x-text="item.rejection_reason"></p></td>
                            <td class="whitespace-nowrap px-4 py-3 text-slate-500" x-text="shortDate(item.created_at)"></td>
                            <td class="whitespace-nowrap px-4 py-3 text-right">
                    ...
                    <tr x-show="!items.length"><td colspan="7" class="px-3 py-10 text-center text-sm text-slate-400">Belum ada pendaftaran sidang TA.</td></tr>
                </tbody>
            </table>
        </div>
