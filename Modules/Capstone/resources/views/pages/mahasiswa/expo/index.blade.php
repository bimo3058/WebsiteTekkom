...
<div x-data="capstoneExpo" class="space-y-6">
    ...
    <div x-show="!loading && !error" x-cloak>
        ...
        <div class="grid gap-4 md:grid-cols-2"><template x-for="event in events" :key="event.id"><x-capstone::card ::class="event.is_registered ? 'border-primary' : full(event) ? 'opacity-60' : ''">
            <div class="px-6 pb-3 flex items-center justify-between gap-2"><h2 class="text-lg font-semibold" x-text="event.name"></h2><x-capstone::badge x-show="event.is_registered" class="bg-green-600"><x-capstone::icon name="CircleCheck" class="h-3 w-3 mr-1" />Registered</x-capstone::badge><x-capstone::badge x-show="!event.is_registered && full(event)" variant="destructive"><x-capstone::icon name="CircleAlert" class="h-3 w-3 mr-1" />Full</x-capstone::badge><x-capstone::badge variant="outline" x-show="!event.is_registered && !full(event)" x-text="(event.capacity-event.registrations_count)+' slots left'" /></div>
