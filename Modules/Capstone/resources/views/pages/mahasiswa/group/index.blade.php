...
<div x-show="!loading && !error" x-cloak class="space-y-6">
    ...
    <template x-if="group"><x-capstone::card>
        ...
        <div class="px-6 space-y-6">
            ...
            <div x-show="!group.title && (group.bids||[]).some(b=>b.lecturer_recommendation==='ACCEPT' && b.status!=='ACCEPTED')" class="p-4 bg-green-50/60 rounded-lg border border-green-200 flex items-start gap-3"><x-capstone::icon name="CheckCircle" class="h-5 w-5 mt-0.5 text-green-700" /><div><h3 class="font-medium" x-text="(group.bids||[]).filter(b=>b.lecturer_recommendation==='ACCEPT' && b.status!=='ACCEPTED').map(b=>b.title?.title).join(', ')"></h3><p class="text-sm text-muted-foreground">Direkomendasi dosen — menunggu <em>Mark Ready for Finalization</em> + penetapan judul oleh koordinator. Judul resmi muncul di sini setelah finalisasi admin.</p></div></div>
            <div x-show="!group.title && !(group.bids||[]).some(b=>b.lecturer_recommendation==='ACCEPT' && b.status!=='ACCEPTED')" class="p-4 bg-muted/50 rounded-lg border border-dashed flex items-center gap-3 text-muted-foreground"><x-capstone::icon name="PenLine" /><span>No title assigned yet. Browse the <x-capstone::feature-link href="/mahasiswa/titles" class="underline text-primary">Titles Marketplace</x-capstone::feature-link> to bid on a title.</span></div>
            @include('capstone::pages.mahasiswa.group.members')
            @include('capstone::pages.mahasiswa.group.join-requests')
        </div>
        <div class="px-6 flex flex-col items-start gap-4 border-t pt-6"><x-capstone::alert x-show="members.length < minMembers && !group.title_id" title="Need More Members">You need at least <span x-text="minMembers"></span> members to bid on a title. Current: <span x-text="members.length+'/'+minMembers"></span></x-capstone::alert><x-capstone::alert title="Title Retained — Add Members" variant="destructive" icon="Lock" x-show="members.length < minMembers && group.title_id">Anggota keluar/dikeluarkan sehingga tersisa <span x-text="members.length+'/'+minMembers"></span>. Judul yang disetujui dosen tetap dipertahankan, tetapi kelompok belum bisa <em>Mark Ready for Finalization</em>. Tambah anggota hingga minimum.</x-capstone::alert><x-capstone::alert title="Over Maximum Members" variant="destructive" icon="Lock" x-show="members.length > maxMembers">Jumlah anggota saat ini <span x-text="members.length+'/'+maxMembers"></span> melebihi batas maksimum. Judul/bid tetap dipertahankan, tetapi finalisasi diblokir. Kurangi anggota hingga maksimum.</x-capstone::alert><x-capstone::button x-show="actions.can_mark_ready_for_finalization" class="w-full" size="lg" @click="confirm('ready')">Mark Ready for Finalization</x-capstone::button><x-capstone::button x-show="actions.can_cancel_ready_for_finalization" variant="outline" class="w-full" size="lg" @click="confirm('cancel')">Cancel Ready for Finalization</x-capstone::button></div>
