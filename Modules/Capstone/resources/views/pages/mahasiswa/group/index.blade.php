...
<div x-show="!loading && !error" x-cloak class="space-y-6">
    ...
    <template x-if="group"><x-capstone::card>
        ...
        <div class="px-6 space-y-6">
            ...
            <div x-show="!group.title && (group.bids||[]).some(b=>b.lecturer_recommendation==='ACCEPT' && b.status!=='ACCEPTED')" class="p-4 bg-green-50/60 rounded-lg border border-green-200 flex items-start gap-3"><x-capstone::icon name="CircleCheck" class="h-5 w-5 mt-0.5 text-green-700" /><div><h3 class="font-medium" x-text="(group.bids||[]).filter(b=>b.lecturer_recommendation==='ACCEPT' && b.status!=='ACCEPTED').map(b=>b.title?.title).join(', ')"></h3><p class="text-sm text-muted-foreground">Direkomendasi dosen — menunggu <em>Mark Ready for Finalization</em> + penetapan judul oleh koordinator. Judul resmi muncul di sini setelah finalisasi admin.</p></div></div>
