...
class AutoMatchmakerService
{
    ...
    public function executeMatchmaking(int $periodId, int $adminId): array
    {
        return DB::transaction(function () use ($periodId, $adminId) {
            ...
            foreach ($incompleteGroups as $group) {
                /** @var \Modules\Capstone\Models\Group $group */
            ...
            foreach ($incompleteGroups as $group) {
                ...
                \Modules\Capstone\Models\Title::where('proposed_by_group_id', $group->id)->update([
            ...
            // Audit
            \Modules\Capstone\Models\AuditLog::create([
                'user_id' => $adminId,
    ...
    private function promoteGroupToReady(Group $group): void
    {
        ...
        $preApprovedTitle = \Modules\Capstone\Models\Title::where('proposed_by_group_id', $group->id)
        ...
        if ($preApprovedTitle) {
            ...
            $currentLoad = \Modules\Capstone\Models\Supervision::where('supervisor_id', $lecturerId)
                ->whereHas('group', fn ($q) => $q->where('period_id', $group->period_id))
                ->count();

            $approvedProposals = \Modules\Capstone\Models\Title::where('proposed_supervisor_id', $lecturerId)
