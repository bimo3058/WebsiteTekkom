...
class BursaIdeController extends Controller
{
    ...
    public function __construct(GroupStateMachine $stateMachine, GroupService $groupService)
    {
    ...
    public function index(Request $request)
    {
        ...
        $userMembership = GroupMember::where('student_id', $studentId)
        ...
        $myPendingRequests = JoinRequest::where('requester_id', $studentId)
    ...
    public function requestJoin(Request $request, $groupId)
    {
        ...
        $sourceMembership = GroupMember::where('student_id', $studentId)
            ->where('period_id', $group->period_id)
            ->first();
        ...
        if ($leader) {
            Notification::create([
                'user_id' => $leader->student->user_id,
                'type' => 'JOIN_REQUEST',
    ...
    public function acceptRequest(Request $request, $id)
    {
        ...
        $leaderMembership = GroupMember::where('student_id', CapstoneActor::student($user)->id)
        ...
        if ($group->is_solo && $group->title_id) {
            $title = \Modules\Capstone\Models\Title::find($group->title_id);
    ...
    public function rejectRequest(Request $request, $id)
    {
        ...
        // Notify requester
        Notification::create([
            'user_id' => $joinRequest->requester->user_id,
