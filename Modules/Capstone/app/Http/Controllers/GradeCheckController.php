...
use Modules\Capstone\Http\Requests\Admin\GradeCheckQueryRequest;
...
class GradeCheckController extends Controller
{
    ...
    private function getStudentAllData(Request $request, $periodId)
    {
        ...
        $periodGroupIds = null;
        if ($periodId) {
            $periodGroupIds = \Modules\Capstone\Models\Group::where('period_id', $periodId)->pluck('id');
        }
        ...
        $schedules = \Modules\Capstone\Models\Schedule::whereIn('group_id', $scoreGroupIds)
    ...
    private function getEvaluatorRole($score, $schedules = null): string
    {
        ...
        if (in_array($evaluationType, ['SEMPRO', 'SIDANG_TA'])) {
            ...
            if ($schedules !== null) {
                ...
                $schedule = \Modules\Capstone\Models\Schedule::where('group_id', $group->id)
    ...
    public function export(Request $request)
    {
        ...
        $groupIds = null;
        if ($periodId) {
            $groupIds = \Modules\Capstone\Models\Group::where('period_id', $periodId)->pluck('id');
        }

        // Get all student IDs first to pre-load student and group data
        $studentIds = collect();
        if ($groupIds && $groupIds->isNotEmpty()) {
            $studentIds = \Modules\Capstone\Models\GroupMember::whereIn('group_id', $groupIds)
                ->pluck('student_id')
                ->unique()
                ->values();
        }
        ...
        $groups = $groupIds && $groupIds->isNotEmpty()
            ? \Modules\Capstone\Models\Group::whereIn('id', $groupIds)
                ->with(['title:id,title'])
        ...
        $studentGroups = ($studentIds->isNotEmpty() && $groupIds && $groupIds->isNotEmpty())
            ? \Modules\Capstone\Models\GroupMember::whereIn('student_id', $studentIds)
    ...
    public function evaluationDetail(Request $request, $studentId, $evaluationType)
    {
        ...
        if ($evaluationType === 'PEER_REVIEW') {
            ...
            if ($groupIds->isNotEmpty()) {
                $schedules = \Modules\Capstone\Models\Schedule::whereIn('group_id', $groupIds)
                    ->whereIn('type', ['SEMPRO', 'SIDANG_TA'])
                    ->get()
                    ->keyBy(fn ($s) => $s->group_id.'_'.$s->type);
            }
