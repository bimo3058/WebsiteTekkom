...
class StudentStateController extends Controller
{
    ...
    public function forceUnlockTA(Request $request, int $studentId): JsonResponse
    {
        $membership = GroupMember::with('group')->where('student_id', $studentId)->firstOrFail();

        $status = StudentPeerReviewStatus::updateOrCreate(
            [
                'student_id' => $studentId,
                'group_id' => $membership->group_id,
                'period_id' => $membership->group->period_id,
            ],
            [
        ...
        return $this->successResponse($status, 'Akses TA mahasiswa berhasil dibuka.');
    }
    ...
    public function getMyTAStatus(Request $request): JsonResponse
    {
        ...
        $member = GroupMember::where('student_id', $studentId)
            ->with('group')
            ->first();
