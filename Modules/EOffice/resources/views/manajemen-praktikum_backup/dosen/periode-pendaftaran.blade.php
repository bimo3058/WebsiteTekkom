<x-eoffice::manajemen-praktikum.layout pageTitle="Periode Pendaftaran Koordinator">
    ...
    <div class="mp-card" style="flex-shrink:0;">
        ...
        <div style="padding:24px;">
            ...
            @else
                <form method="GET" action="{{ route('eoffice.manprak.dosen.periode-pendaftaran.index') }}"
                    id="form-praktikum">
    ...
    @if($praktikumDipilih)
        ...
        @if($praktikumDipilih->koor_id)
            <div class="mp-alert info flex-shrink-0" style="margin-top:24px;">
                Praktikum ini sudah memiliki Koordinator ({{ $praktikumDipilih->koordinator->name }}).
            </div>
        @else
            {{-- ── BUKA PERIODE BARU ────────────────────────────────────────────────────── --}}
                                ...
                                <tr style="border-bottom:1px solid #F3F4F6; transition:background .12s;"
                                    ...
                                    <td style="padding:12px 16px; text-align:center;">
                                        <div style="display:flex;gap:6px;justify-content:center;">
                                            @if($p->is_aktif)
                                                <form method="POST"
                                                    action="{{ route('eoffice.manprak.dosen.periode-pendaftaran.tutup', $p->id) }}"
                                            ...
                                            @endif
                                            <form method="POST"
                                                action="{{ route('eoffice.manprak.dosen.periode-pendaftaran.destroy', $p->id) }}"
                        ...
                        <tbody>
                            @foreach($riwayatPengumuman as $pg)
                                <tr style="border-bottom:1px solid #F3F4F6; transition:background .12s;"
                                    ...
                                    <td style="padding:12px 16px; font-size:12px; color:var(--c-fg-muted, #666D80);">
                                        {{ $pg->created_at->locale('id')->isoFormat('D MMM YYYY, HH:mm') }}</td>
                                    <td style="padding:12px 16px; text-align:center;">
                                        @if($pg->trashed())
