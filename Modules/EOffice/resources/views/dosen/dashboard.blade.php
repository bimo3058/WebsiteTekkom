...
<div class="flex h-screen w-full overflow-hidden">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-30 w-72 bg-white border-r border-slate-200 flex flex-col transition-transform duration-300 lg:static lg:translate-x-0 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
        ...
        <div class="flex-1 overflow-y-auto py-6 px-4">
            ...
            <a href="{{ route('eoffice.kp.dosen.dashboard') }}" class="flex items-center px-4 py-3 mb-1 text-indigo-700 bg-indigo-50/50 rounded-xl transition-all text-sm font-semibold relative before:absolute before:left-0 before:top-2 before:bottom-2 before:w-1 before:bg-indigo-600 before:rounded-r-full">
            ...
            <div class="max-w-6xl mx-auto">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-8 p-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                        <!-- Stat 1 -->
                        <div>
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Total Bimbingan</p>
                            <div class="flex items-end gap-3">
                                <p class="text-5xl font-extrabold text-slate-900 tracking-tight">{{ $stats['total_bimbingan'] ?? 0 }}</p>
                        ...
                        <!-- Stat 2 -->
                        <div>
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Menunggu ACC</p>
                            <div class="flex items-end gap-3">
                                <p class="text-5xl font-extrabold text-amber-500 tracking-tight">{{ $stats['menunggu_acc'] ?? 0 }}</p>
                        ...
                        <div>
                            ...
                            <div class="flex items-end gap-3">
                                <p class="text-5xl font-extrabold text-indigo-600 tracking-tight">{{ $stats['sedang_kp'] ?? 0 }}</p>
                                <p class="text-sm text-slate-500 font-medium pb-1.5">Mahasiswa</p>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-end gap-3">
                                <p class="text-5xl font-extrabold text-emerald-500 tracking-tight">{{ $stats['selesai_kp'] ?? 0 }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 flex flex-col group hover:border-indigo-200 hover:shadow-md transition-all">
                        ...
                        <h3 class="text-xl font-bold text-slate-900 mb-3 group-hover:text-indigo-700 transition-colors">Bimbingan Mahasiswa</h3>
                        <p class="text-sm text-slate-500 flex-1 mb-8 leading-relaxed">Kelola daftar mahasiswa bimbingan, pantau progres, lihat riwayat bimbingan, dan berikan nilai seminar akhir Kerja Praktik.</p>
                        <a href="{{ route('eoffice.kp.dosen.bimbingan.index') }}" class="inline-flex items-center justify-center px-5 py-3 bg-indigo-600 text-white font-bold text-sm rounded-xl hover:bg-indigo-700 transition-colors shadow-sm shadow-indigo-200 focus:ring-4 focus:ring-indigo-100 outline-none w-fit">
                            Kelola Bimbingan
