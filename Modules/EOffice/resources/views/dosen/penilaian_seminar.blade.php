...
<div class="flex h-screen w-full overflow-hidden bg-slate-50/50">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-30 w-72 bg-white border-r border-slate-200 flex flex-col transition-transform duration-300 lg:static lg:translate-x-0 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
        ...
        <div class="flex-1 overflow-y-auto py-6 px-4">
            ...
            <a href="{{ route('eoffice.kp.dosen.penilaian_seminar') }}" class="flex items-center px-4 py-3 mb-1 text-indigo-700 bg-indigo-50/50 rounded-xl transition-all text-sm font-semibold relative before:absolute before:left-0 before:top-2 before:bottom-2 before:w-1 before:bg-indigo-600 before:rounded-r-full">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50/50">
        ...
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-6 lg:px-10 z-10 sticky top-0">
            <div class="flex items-center">
                ...
                <nav class="hidden sm:flex items-center space-x-2 text-sm text-slate-500 font-medium">
                    <span class="text-indigo-700 font-semibold bg-indigo-50 px-2.5 py-1 rounded-md">Penilaian Seminar</span>
                </nav>
            </div>
        ...
        <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
            ...
            <div class="mb-6">
                ...
                <p class="text-sm text-slate-500 mt-1">Tinjau jadwal seminar yang diajukan dan berikan nilai seminar mahasiswa.</p>
            </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-12 text-center">
                    ...
                    <p class="text-xs text-slate-500 mt-1">Mahasiswa bimbingan Anda belum mengajukan jadwal seminar KP.</p>
                </div>
                @else
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th class="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Jadwal Seminar</th>
                                <th class="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Tempat / Ruangan</th>
                                <th class="px-5 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">Penilaian</th>
                                <th class="px-5 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">Approval Jadwal</th>
                            </tr>
                        </thead>
                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="px-5 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-sm font-medium text-slate-900">{{ $sem->nama_mahasiswa ?? 'Mahasiswa' }}</p>
                                ...
                                <td class="px-5 py-4 whitespace-nowrap">
                                    ...
                                    <p class="text-xs text-slate-500 mt-0.5">
                                        ...
                                        @endphp
                                        Jam: {{ $startTime ? \Carbon\Carbon::parse($startTime)->format('H:i') : '-' }} - {{ $endTime ? $endTime : '-' }}
                                    </p>
                                </td>
                                <td class="px-5 py-4 whitespace-nowrap">
                                    <span class="text-sm text-slate-700">{{ $sem->ruangan ?? 'Belum ditentukan' }}</span>
                                </td>
                                <td class="px-5 py-4 whitespace-nowrap text-center">
                                    @if($sem->nilai_seminar_pembimbing !== null)
                                        <span class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-emerald-50 text-emerald-700 font-extrabold text-lg border border-emerald-100 shadow-sm" @click="openDrawer({{ $sem->kp_id }}, '{{ $sem->nama_mahasiswa }}', '{{ $sem->nim }}', {{ $sem->nilai_seminar_pembimbing }})" style="cursor: pointer;" title="Edit Nilai">
                                            {{ $sem->nilai_seminar_pembimbing }}
                                        </span>
                                    @else
                                        <button @click="openDrawer({{ $sem->kp_id }}, '{{ $sem->nama_mahasiswa }}', '{{ $sem->nim }}', null)" class="inline-flex items-center px-3 py-1.5 bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 rounded-lg text-xs font-semibold transition-colors">
                                            Beri Nilai
                                        </button>
                                    @endif
                                </td>
                                <td class="px-5 py-4 whitespace-nowrap text-center">
                                    @if($sem->status_validasi_dosen == 'pending')
                                        <div class="flex items-center justify-center gap-2">
                                            <form action="{{ route('eoffice.kp.dosen.penilaian_seminar.reject', $sem->kp_id) }}" method="POST">
                                                @csrf
                                                <button type="submit" title="Tolak" class="p-1.5 text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg transition-colors">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                                                </button>
                                            </form>
                                            <form action="{{ route('eoffice.kp.dosen.penilaian_seminar.approve', $sem->kp_id) }}" method="POST">
                                                @csrf
                                                <button type="submit" title="Setujui" class="p-1.5 text-emerald-600 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-lg transition-colors">
                                    ...
                                    @elseif($sem->status_validasi_dosen == 'rejected')
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold bg-red-100 text-red-800 border border-red-200">
                    ...
                    <div x-show="drawerOpen" 
                        ...
                        <div class="flex h-full flex-col overflow-y-scroll bg-white shadow-2xl border-l border-slate-200">
                            ...
                            <div class="px-6 py-6 border-b border-slate-100 bg-slate-50/50 sticky top-0 z-10 backdrop-blur-md flex items-center justify-between">
                                <div>
                                    <h2 class="text-xl font-bold text-slate-900" id="slide-over-title">Form Penilaian Seminar</h2>
                            ...
                            <div class="flex-1 px-6 py-6 space-y-6">
                                ...
                                <div class="flex items-center gap-4">
                                    <div class="h-12 w-12 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-bold text-lg border-2 border-white shadow-sm flex-shrink-0" x-text="selectedStudentName.charAt(0).toUpperCase()">
                                    </div>
                                    <div>
                                        <h1 class="text-lg font-bold text-slate-900" x-text="selectedStudentName"></h1>
                                        <p class="text-sm text-slate-500">NIM: <span class="font-semibold text-slate-700" x-text="selectedStudentNim"></span></p>
                                    </div>
                                </div>
                                ...
                                <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-blue-600 mt-1">Bagian ini hanya untuk <strong>Nilai Seminar Pembimbing</strong>. Nilai Lapangan diisi oleh Koordinator KP.</p>
                                    </div>
                                </div>

                                <form :action="'/eoffice/kp/dosen/bimbingan/' + selectedKpId + '/penilaian'" method="POST">
                                    @csrf
                                    <div>
                                        ...
                                        <input
                                            type="number"
                                            id="nilai_seminar_pembimbing"
                                            name="nilai_seminar_pembimbing"
                                            min="0" max="100" step="0.01"
                                            x-model="selectedNilai"
                                    ...
                                    <div class="mt-8 flex gap-3">
                                        <button type="button" @click="closeDrawer()" class="flex-1 px-6 py-3 border border-slate-300 text-slate-700 font-medium rounded-xl hover:bg-slate-50 transition-colors text-sm text-center">
                                            Batal
                                        </button>
