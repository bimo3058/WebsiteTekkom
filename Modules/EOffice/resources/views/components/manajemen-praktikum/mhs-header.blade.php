...
<div x-data="{ st: 0 }" x-init="
    ...
    <div
        ...
        <div class="absolute inset-0 flex items-center justify-center bg-[#F3F4F6]" style="border-radius: inherit;">
            @if($praktikum && $praktikum->cover_path)
    ...
    <div style="display: flex; gap: 8px; overflow-x: auto;" class="no-scrollbar">
        <a href="{{ route('eoffice.manprak.mahasiswa.pengumuman.index', ['praktikum_id' => $prakId]) }}"
        ...
        <a href="{{ route('eoffice.manprak.mahasiswa.modul.index', ['praktikum_id' => $prakId]) }}"
        ...
        <a href="{{ route('eoffice.manprak.mahasiswa.tugas.index', ['praktikum_id' => $prakId]) }}"
        ...
        <a href="{{ route('eoffice.manprak.mahasiswa.nilai.index', ['praktikum_id' => $prakId]) }}"
        ...
        <a href="{{ route('eoffice.manprak.mahasiswa.daftar-praktikan.index', ['praktikum_id' => $prakId]) }}"
            style="padding: 12px 24px; font-weight: {{ $isPraktikan ? '600' : '500' }}; font-size: 14px; color: {{ $isPraktikan ? '#293C79' : 'var(--c-fg-muted)' }}; text-decoration: none; {!! $isPraktikan ? 'border-bottom: 2px solid #293C79;' : '' !!}">Daftar Praktikan</a>
    </div>
</div>
