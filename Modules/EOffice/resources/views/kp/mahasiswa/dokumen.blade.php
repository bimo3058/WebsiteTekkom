    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
            ...
            @php
                ...
                $step3 = $dokumenByJenis->has('A2');
                $step4 = $dokumenByJenis->has('Laporan') && $dokumenByJenis->has('Makalah');
                ...
                <div class="relative">
                    ...
                    <div class="relative z-10 flex justify-between">
                        @foreach([
                            ...
                            ['label' => 'Logbook A2', 'icon' => 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z'],
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @php
                                $uploadItems = [
                                    ...
                                    ['id' => 'step-3', 'key' => 'A2', 'display_name' => 'Presensi KP (A2)', 'number' => '3', 'icon' => 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z', 'desc' => 'Borang logbook harian & penilaian lapangan.', 'color' => 'amber'],
                                    ['id' => 'step-4-1', 'key' => 'Laporan', 'display_name' => 'Laporan Akhir', 'number' => '4', 'icon' => 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253', 'desc' => 'Draf Laporan KP versi terakhir (PDF).', 'color' => 'indigo'],
                                    ['id' => 'step-4-2', 'key' => 'Makalah', 'display_name' => 'Makalah IEEE', 'number' => '4', 'icon' => 'M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z', 'desc' => 'Makalah format IEEE (PDF).', 'color' => 'rose'],
                                ];
                            @endphp
                            ...
                            @php
                                ...
                                if ($jenis === 'A2' && $currentStep === 3) $isActiveCard = true;
                                if (($jenis === 'Laporan' || $jenis === 'Makalah') && $currentStep === 4) $isActiveCard = true;
                            @endphp
                            ...
                            <div id="{{ $item['id'] }}" class="bg-white rounded-2xl border p-5 flex flex-col h-full transition-all duration-300 relative {{ $isActiveCard ? 'border-indigo-500 ring-4 ring-indigo-200 shadow-lg shadow-indigo-100/50 z-10' : 'border-slate-100 shadow-sm hover:shadow-md' }}">
                                ...
                                <div class="mb-4 flex items-center gap-2 p-2 rounded-lg bg-slate-50 border border-slate-100">
                                    ...
                                    <a href="{{ Storage::url($latestDoc->file_path) }}" target="_blank" class="text-[10px] font-bold text-indigo-600 hover:text-indigo-700">LIHAT</a>
                                </div>
                                @endif
                ...
                <div class="space-y-8">
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <div class="p-5 space-y-4" x-data="{ modalA2: false }">
                            @foreach([
                                ...
                                ['type' => 'a2', 'title' => 'Presensi KP (A2)', 'desc' => 'Logbook & Nilai (.docx)', 'color' => 'emerald'],
                            ] as $tm)
                            
                            @if($tm['type'] === 'a2')
                                <button type="button" @click="modalA2 = true" class="w-full text-left flex items-center p-3 rounded-xl border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50 transition-all group">
                            ...
                            {{-- Modal Form A2 --}}
                            <div x-show="modalA2" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
                                <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                                    <div class="fixed inset-0 transition-opacity" @click="modalA2 = false">
                                        <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm"></div>
                                    </div>
                                    <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
                                    <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full">
                                        <form action="{{ route('eoffice.kp.mahasiswa.dokumen.export_a2') }}" method="POST">
                                            @csrf
                                            <div class="bg-white p-6">
                                                <div class="mb-5">
                                                    <h3 class="text-lg leading-6 font-bold text-slate-900">Buat Presensi KP (A2)</h3>
                                                    <p class="text-xs text-slate-500 mt-1">Lengkapi data Pembimbing Lapangan Anda untuk men-generate form A2.</p>
                                                </div>
                                                
                                                <div class="space-y-4">
                                                    <div>
                                                        <label class="block text-xs font-bold text-slate-700 mb-1">Nama Pembimbing Lapangan <span class="text-red-500">*</span></label>
                                                        <input type="text" name="nama" required class="w-full rounded-lg px-3 py-2 text-sm border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 bg-slate-50">
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-bold text-slate-700 mb-1">NIP / NIK Pembimbing <span class="text-red-500">*</span></label>
                                                        <input type="text" name="nip" required class="w-full rounded-lg px-3 py-2 text-sm border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 bg-slate-50">
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-bold text-slate-700 mb-1">Jabatan <span class="text-red-500">*</span></label>
                                                        <input type="text" name="jabatan" required placeholder="Contoh: Manajer IT" class="w-full rounded-lg px-3 py-2 text-sm border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 bg-slate-50">
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-bold text-slate-700 mb-1">Perusahaan / Instansi <span class="text-red-500">*</span></label>
                                                        <input type="text" name="perusahaan" required value="{{ $kp->tempat_fix }}" class="w-full rounded-lg px-3 py-2 text-sm border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 bg-slate-50">
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="bg-slate-50 px-6 py-4 flex flex-row-reverse gap-3">
                                                <button type="submit" @click="setTimeout(() => modalA2 = false, 500)" class="inline-flex justify-center rounded-xl px-5 py-2 bg-indigo-600 text-sm font-bold text-white hover:bg-indigo-700 shadow-md transition-all active:scale-95">
                                                    Unduh Dokumen (.docx)
                                                </button>
                                                <button type="button" @click="modalA2 = false" class="inline-flex justify-center rounded-xl px-5 py-2 bg-white border border-slate-200 text-sm font-bold text-slate-600 hover:bg-slate-100 transition-all">
                                                    Batal
                                                </button>
                                            </div>
                                        </form>
