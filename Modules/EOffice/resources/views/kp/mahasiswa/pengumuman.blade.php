    ...
    <style>
        * { font-family: 'Inter Tight', sans-serif; }
        :root {
            ...
            --warning-50:#fef3c7;--warning-300:#d97706;
    ...
    <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
            ...
            <div class="mb-6">
                ...
                <p class="text-sm mt-1" style="color:var(--grey-500);">Informasi terbaru dan timeline kegiatan Kerja Praktik dari Koordinator.</p>
            </div>
            ...
            <div class="sikape-card p-4 mb-6 flex items-center gap-3">
                ...
                <input type="text" x-model="search" placeholder="Cari pengumuman..." class="flex-1 text-sm outline-none bg-transparent" style="color:var(--grey-800);">
            ...
            <div class="space-y-4">
                ...
                <div x-show="('{{ addslashes($item->judul) }}'.toLowerCase().includes(search.toLowerCase()) || '{{ addslashes($item->konten) }}'.toLowerCase().includes(search.toLowerCase()) || search === '')"
                    ...
                    <button @click="openItem = openItem === {{ $i }} ? null : {{ $i }}"
                        ...
                        <div class="flex items-start sm:items-center gap-3 flex-1 min-w-0">
                            ...
                            <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full flex-shrink-0" style="background:var(--info-50);color:var(--info-500);">Timeline</span>
                            @else
                            ...
                            <div class="flex-1 min-w-0 mt-1 sm:mt-0">
                                <p class="text-sm font-semibold pr-2" style="color:var(--grey-800);">{{ $item->judul }}</p>
                                <div class="flex items-center flex-wrap gap-2 mt-0.5">
                                    <p class="text-xs" style="color:var(--grey-400);">{{ \Carbon\Carbon::parse($item->updated_at)->translatedFormat('d F Y, H:i') }} WIB</p>
                                    <span class="text-[10px]" style="color:var(--grey-300);">•</span>
                                    <p class="text-xs" style="color:var(--grey-400);">{{ $item->pembuat->name ?? 'Admin' }}</p>
                                    @if($item->updated_at->ne($item->created_at))
                                    <span class="text-[9px] font-bold px-1.5 py-0.5 rounded" style="background:var(--grey-100);color:var(--grey-500);">Diedit</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center self-end sm:self-auto gap-2">
                            <span class="text-xs" style="color:var(--grey-400);" x-text="openItem === {{ $i }} ? 'Tutup' : 'Lihat Detail'"></span>
                ...
                <div class="sikape-card p-12 text-center" x-show="search === ''">
                    <svg class="w-12 h-12 mx-auto mb-3" style="color:var(--grey-300);" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                    <p class="text-sm font-semibold" style="color:var(--grey-500);">Belum ada pengumuman</p>
                    <p class="text-xs mt-1" style="color:var(--grey-400);">Pengumuman terbaru dari Koordinator KP akan muncul di sini.</p>
                </div>
                @endforelse
                ...
                <div x-show="search && search.length > 0" class="hidden last:block">
                    <div class="sikape-card p-12 text-center">
                        ...
                        <p class="text-sm font-semibold" style="color:var(--grey-500);">Pengumuman tidak ditemukan</p>
                        <p class="text-xs mt-1" style="color:var(--grey-400);">Tidak ada pengumuman yang cocok dengan pencarian Anda.</p>
                    </div>
                </div>
