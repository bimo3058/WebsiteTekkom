<x-eoffice::manajemen-praktikum.layout pageTitle="Absensi &amp; Nilai">
    ...
    <div class="mp-page-header">
        ...
        <div class="mp-page-actions">
            <a href="{{ route('eoffice.manprak.asprak.absensi.index') }}" class="mp-btn secondary md"
    ...
    <div class="mp-card flex-shrink-0"
        style="margin-top:24px;margin-bottom:24px;padding:14px 20px;background:#EFF6FF;border-left:3px solid #3B82F6;">
        <p style="font-size:12px;color:#1E3A8A;margin:0;line-height:1.6;">
            ...
            Kolom Nilai (Tugas Pendahuluan, Praktikum, Laporan, Responsi) akan otomatis aktif jika mahasiswa ditandai
            <strong>Hadir</strong>.
            Nilai yang diinput di sini akan tersinkronisasi dua arah dengan form tugas di menu <em>Lihat
                Pengumpulan</em>.
        </p>
    </div>

    {{-- Single Form for Absensi & Nilai --}}
    <form method="POST" action="{{ route('eoffice.manprak.asprak.absensi.store', $modul->id) }}"
        class="mp-card flex-shrink-0" x-data="{ search: '', kelompok: '', shift: '' }">
        @csrf
        <div class="mp-card-header"
            style="position:sticky;left:0; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px; z-index:20;">
            ...
            <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
                <input type="text" x-model="search" placeholder="Cari nama atau NIM..." class="mp-input"
                    style="padding:6px 10px; font-size:13px; width:200px;">
                ...
                <div x-data="{
                    ...
                    <div class="mp-input flex items-center justify-between cursor-pointer"
                        style="padding:6px 10px; font-size:13px; width:150px; background:#fff;"
                        @click="open = !open; if(open) $nextTick(() => $refs.searchKel.focus())">
                        <span x-text="kelompok === '' ? 'Semua Kelompok' : kelompok" class="truncate"
                            :style="kelompok === '' ? 'color:#808897' : 'color:#0D0D12; font-weight:500;'"></span>
                    ...
                    <div x-show="open" x-transition
                        ...
                        <div style="overflow-y:auto; flex:1; max-height: 200px;">
                            <div @click="selectOption('')"
                                ...
                                :style="kelompok === '' ? 'background:#EAF0FA; color:#0D1D53; font-weight:600;' : 'color:#353849;'"
                                onmouseover="if(kelompok !== '') this.style.background='#F6F8FA'"
                                onmouseout="if(kelompok !== '') this.style.background='transparent'">
                                Semua Kelompok
                            </div>
                            <template x-for="opt in filtered" :key="opt">
                                <div @click="selectOption(opt)"
                                    class="px-3 py-[8px] text-[13px] cursor-pointer transition-colors"
                                    :style="kelompok === opt ? 'background:#EAF0FA; color:#0D1D53; font-weight:600;' : 'color:#353849;'"
                                    onmouseover="if(kelompok !== opt) this.style.background='#F6F8FA'"
                                    onmouseout="if(kelompok !== opt) this.style.background='transparent'" x-text="opt">
                                </div>
                            </template>
                ...
                <div x-data="{
                    ...
                    <div class="mp-input flex items-center justify-between cursor-pointer"
                        ...
                        <span x-text="shift === '' ? 'Semua Shift' : shift" class="truncate"
                            :style="shift === '' ? 'color:#808897' : 'color:#0D0D12; font-weight:500;'"></span>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#A4ABB8" stroke-width="2"
                            stroke-linecap="round" stroke-linejoin="round"
                            :style="open ? 'transform:rotate(180deg)' : ''" style="transition:transform 0.2s;">
                            <polyline points="6 9 12 15 18 9"></polyline>
                    ...
                    <div x-show="open" x-transition
                        ...
                        <div style="overflow-y:auto; flex:1; max-height: 200px;">
                            <div @click="selectOption('')"
                                class="px-3 py-[8px] text-[13px] cursor-pointer transition-colors"
                                :style="shift === '' ? 'background:#EAF0FA; color:#0D1D53; font-weight:600;' : 'color:#353849;'"
                                onmouseover="if(shift !== '') this.style.background='#F6F8FA'"
                                onmouseout="if(shift !== '') this.style.background='transparent'">
                                Semua Shift
                            </div>
                            <template x-for="opt in filtered" :key="opt">
                                <div @click="selectOption(opt)"
                                    class="px-3 py-[8px] text-[13px] cursor-pointer transition-colors"
                                    :style="shift === opt ? 'background:#EAF0FA; color:#0D1D53; font-weight:600;' : 'color:#353849;'"
                                    onmouseover="if(shift !== opt) this.style.background='#F6F8FA'"
                                    onmouseout="if(shift !== opt) this.style.background='transparent'" x-text="opt">
                                </div>
                            </template>
                            <div x-show="filtered.length === 0" class="px-3 py-[12px] text-[12px] text-center"
                                style="color:#A4ABB8;">
                ...
                <thead>
                    <tr style="background:#F9FAFB;">
                        <th class="mp-th text-left"
                            style="padding:10px 16px;width:40px;position:sticky;left:0;background:#F9FAFB;z-index:2;">NO
                        </th>
                        <th class="mp-th text-left"
                            style="padding:10px 16px;min-width:200px;position:sticky;left:40px;background:#F9FAFB;z-index:2;border-right:1px solid #DFE1E7;">
                            NAMA MAHASISWA</th>
                        <th class="mp-th text-center"
                            style="padding:10px 16px;width:90px;border-right:1px solid #DFE1E7;">KELOMPOK</th>
                        <th class="mp-th text-center"
                            style="padding:10px 16px;width:70px;border-right:1px solid #DFE1E7;">Shift</th>
                            ...
                            <td
                                ...
                                <div style="display:flex;align-items:center;gap:10px;">
                                    <div class="mp-av yellow" style="flex-shrink:0;">
                                        {{ strtoupper(substr($p->user?->name ?? 'M', 0, 2)) }}</div>
                                    <div style="min-width:0;">
                                        <div
                                            style="font-weight:600;color:#0D0D12;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                                            {{ $p->user?->name ?? '-' }}</div>
                                        <div style="font-size:11px;color:#666D80;">
                                            {{ $p->user?->student?->student_number ?? '-' }}</div>
                                    </div>
                                </div>
                            </td>
