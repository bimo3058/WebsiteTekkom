...
<div class="mp-page-header">
    <div>
        ...
        <p class="mp-page-sub">Informasi dan pengumuman dari asisten dan koordinator praktikum · {{ now()->locale('id')->isoFormat('dddd, D MMMM YYYY') }}</p>
    </div>
</div>
...
<div class="mp-card flex-shrink-0">
    <div style="padding:14px 18px;">
        <form method="GET" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
            ...
            @if($praktikum)
            <span class="mp-badge warning sm"><span class="dot"></span>{{ $pengumumans instanceof \Illuminate\Pagination\LengthAwarePaginator ? $pengumumans->total() : $pengumumans->count() }} pengumuman</span>
            @endif
        </form>
    </div>
</div>
...
<div class="mp-stats-grid cols-2 flex-shrink-0" style="grid-template-columns:repeat(2,1fr);">
    <div class="mp-stat" style="display:flex;align-items:center;gap:14px;padding:14px 18px;">
        <div class="mp-stat-icon" style="background:rgba(211,156,61,0.12);">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#D39C3D" stroke-width="2" stroke-linecap="round">
                <path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0"/>
            </svg>
        </div>
        <div>
            <div class="mp-stat-value" style="font-size:22px;">{{ $pengumumans instanceof \Illuminate\Pagination\LengthAwarePaginator ? $pengumumans->total() : $pengumumans->count() }}</div>
            <div class="mp-stat-label" style="margin:0;">Total Pengumuman</div>
        </div>
    </div>
    <div class="mp-stat" style="display:flex;align-items:center;gap:14px;padding:14px 18px;">
        <div class="mp-stat-icon" style="background:rgba(64,196,170,0.12);">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#40C4AA" stroke-width="2" stroke-linecap="round">
        ...
        <div>
            ...
            @endphp
            <div class="mp-stat-value" style="font-size:14px;font-weight:600;">{{ $latest ? $latest->created_at?->locale('id')->diffForHumans() : '—' }}</div>
...
<div class="sec-head flex-shrink-0">
    ...
    <span class="sec-rule"></span>
    <span class="mp-badge neutral sm">{{ $pengumumans instanceof \Illuminate\Pagination\LengthAwarePaginator ? $pengumumans->total() : $pengumumans->count() }} total</span>
</div>
    ...
    <div style="padding:20px;">
        <div style="display:flex;align-items:flex-start;gap:14px;">
            ...
            <div style="flex-shrink:0;margin-top:2px;">
                @if($isSistem)
                <div style="width:36px;height:36px;border-radius:10px;background:rgba(11,38,110,0.1);display:flex;align-items:center;justify-content:center;">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#0B266E" stroke-width="2" stroke-linecap="round">
                        <path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0"/>
                    </svg>
                </div>
                @else
                ...
                @endphp
                <div style="width:36px;height:36px;border-radius:10px;background:{{ $bg }};display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;color:#fff;">
                    {{ $initials }}
                </div>
                @endif
            </div>
            ...
            <div style="flex:1;min-width:0;">
                ...
                {{-- Meta --}}
                <div style="font-size:11px;color:#808897;margin-bottom:12px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
                    <span style="display:flex;align-items:center;gap:4px;">
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
                            <path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2M12 11a4 4 0 100-8 4 4 0 000 8z"/>
                        </svg>
                        <strong style="color:#666D80;">{{ $isSistem ? 'Sistem' : ($pg->user?->name ?? '—') }}</strong>
                    </span>
                    <span style="display:flex;align-items:center;gap:4px;">
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
                            <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
