<x-eoffice::manajemen-praktikum.layout pageTitle="Tugas Praktikum">
    ...
    @if($tugasList->isEmpty())
        <div class="mp-card flex-shrink-0" style="padding:40px;text-align:center;">
            <svg class="mx-auto mb-3" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#A4ABB8"
                ...
                <div class="mp-card flex-shrink-0" style="padding:20px;" x-data="{ open: false }">
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-1 flex-wrap">
                                ...
                                @if($statusTugas === 'acc')
                                    <span class="mp-badge success sm"><span
                                            class="dot"></span>ACC{{ $pengumpulan?->nilai ? ' — Nilai: ' . $pengumpulan->nilai : '' }}</span>
                                ...
                                @elseif($lewat)
                                    <span class="mp-badge warning sm"><span class="dot"></span>Tenggat AC Terlewat (Masih Bisa
                                        Kumpul)</span>
                                ...
                                @if($isLate)
                                    <span class="mp-badge error sm"
                                        style="background:#FFF0F2;color:#DF1C41;border:1px solid #DF1C41;">Terlambat (Lewat Deadline
                                        AC)</span>
                                @endif
                            </div>
                            <div style="font-size:12px;color:#666D80;">
                                ...
                                @if($dl)
                                    · Deadline AC: <span
                                        style="font-weight:600;color:{{ $deadlineColor }};">{{ $dl->format('d M Y, H:i') }}</span>
                                @endif
                                @if($dlAcc)
                                    · Deadline ACC: <span
                                        style="font-weight:600;color:{{ $lewatAcc ? '#DF1C41' : '#353849' }};">{{ $dlAcc->format('d M Y, H:i') }}</span>
                                @endif
                            </div>
                            @if(!empty($tugas->deskripsi))
                                <div style="font-size:12px;color:#666D80;margin-top:4px;" class="line-clamp-2">
                                    {{ $tugas->deskripsi }}</div>
                            @endif
                        </div>
                    ...
                    @if($sudahKumpul && $pengumpulan?->file_path)
                        <div class="mt-3 p-3 rounded-[8px]" style="background:#F4F6F8;border:1px solid #DFE1E7;">
                            <div
                                style="font-size:11px;font-weight:700;color:#353849;margin-bottom:6px;display:flex;align-items:center;justify-content:space-between;">
                                <span>File yang Dikumpulkan:</span>
                                <span style="font-size:10px;font-weight:500;color:#666D80;">Update terakhir:
                                    {{ \Carbon\Carbon::parse($pengumpulan->updated_at)->locale('id')->format('d M Y, H:i') }}</span>
                            </div>
                            ...
                            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;">
                                ...
                                @if($pengumpulan->riwayat && $pengumpulan->riwayat->isNotEmpty())
                                    <div x-data="{ openRiwayat: false }" style="position:relative;">
                                        <button type="button" @click="openRiwayat = !openRiwayat" class="mp-btn secondary sm"
                                            style="font-size:11px;padding:4px 8px;display:inline-flex;align-items:center;gap:2px;">
                                        ...
                                        <div x-show="openRiwayat" @click.away="openRiwayat = false"
                                            style="position:absolute;top:100%;right:0;background:#fff;border:1px solid #DFE1E7;border-radius:8px;padding:8px;box-shadow:0 10px 15px -3px rgba(0,0,0,0.1);z-index:100;min-width:240px;display:flex;flex-direction:column;gap:6px;margin-top:4px;">
                                            @foreach($pengumpulan->riwayat as $index => $r)
                                                <a href="{{ app(\App\Services\SupabaseStorage::class)->publicUrl($r->file_path, 'eoffice') }}"
                                                    target="_blank"
                                                    style="font-size:11px;color:#353849;text-decoration:none;display:flex;flex-direction:column;padding:6px;border-radius:6px;transition:background .1s;text-align:left;"
                                                    onmouseover="this.style.background='#F3F4F6'" onmouseout="this.style.background=''">
                                                    <div style="display:flex;justify-content:space-between;align-items:center;">
                                                        <span
                                                            style="font-weight:700;color:#0B266E;">#{{ $pengumpulan->riwayat->count() - $index }}
                                                            {{ $r->is_revision ? 'Revisi' : 'Pertama' }}</span>
                                                        <span style="font-size:9px;color:#888;">{{ $r->created_at->format('H:i') }}</span>
                                                    </div>
                                                    @if($r->catatan)
                                                        <span style="font-size:10px;color:#666D80;margin-top:2px;font-style:italic;"
                                                            class="truncate">💬 {{ $r->catatan }}</span>
                                                    @endif
                                                    <span
                    ...
                    @if($sudahKumpul && ($pengumpulan?->catatan_revisi || $pengumpulan?->file_revisi_asprak))
                        <div class="mt-3 p-3 rounded-[8px]" style="background:#FADAE1;border:1px solid #DF1C41;">
                            <div style="font-size:11px;font-weight:700;color:#7C1028;margin-bottom:2px;">Catatan Revisi dari
                                Asisten:</div>
                            ...
                            @if($pengumpulan->file_revisi_asprak)
                                <div style="margin-top:6px;">
                                    <a href="{{ app(\App\Services\SupabaseStorage::class)->publicUrl($pengumpulan->file_revisi_asprak, 'eoffice') }}"
                                        target="_blank"
                                        style="font-size:11px;font-weight:700;color:#95122B;text-decoration:none;display:inline-flex;align-items:center;gap:4px;">
                    ...
                    <div x-show="open" x-transition class="mt-4 pt-4" style="border-top:1px solid #ECEFF3;">
                        @if($statusTugas === 'revisi')
                            <div style="font-size:12px;font-weight:600;color:#D39C3D;margin-bottom:12px;">Kirim ulang file
                                perbaikan:</div>
                            <form method="POST" action="{{ route('eoffice.manprak.mahasiswa.tugas.kirim-ulang', $tugas->id) }}"
                                enctype="multipart/form-data">
                                @csrf
                        @else
                                <form method="POST" action="{{ route('eoffice.manprak.mahasiswa.tugas.kumpul', $tugas->id) }}"
                                    enctype="multipart/form-data">
                                    @csrf
                            @endif
                                <div class="flex flex-col gap-3">
                                    <div>
                                        <label class="block mb-1" style="font-size:12px;font-weight:600;color:#353849;">File
                                            Tugas <span style="color:#DF1C41;">*</span></label>
                                        <input type="file" name="file" required class="mp-input w-full">
                                        <div style="font-size:11px;color:#666D80;margin-top:4px;">Format: PDF, DOCX, ZIP, RAR
                                            (maks. 10MB)</div>
                                    </div>
                                    <div>
                                        <label class="block mb-1" style="font-size:12px;font-weight:600;color:#353849;">Catatan
                                            (opsional)</label>
