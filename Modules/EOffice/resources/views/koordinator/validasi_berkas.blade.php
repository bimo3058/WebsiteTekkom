<x-eoffice::layouts.koordinator title="Approval Berkas">
    @section('breadcrumbs')
            ...
            function approvalApp() {
                return {
                    ...
                    processApproval(docId, status) {
                        ...
                        this.isLoading = true;
                        fetch(`/eoffice/kp/koordinator/validasi-berkas/${docId}/approve`, { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' } })
                            .then(res => res.json())
                            .then(data => {
                                if (data.success) {
                    ...
                    submitReject() {
                        ...
                        fetch(endpoint, { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify({ revision_note: this.rejectReason }) })
                            .then(res => res.json())
                            .then(data => {
                                if (data.success) {
