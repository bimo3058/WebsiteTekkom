        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-8">
            <div class="border-b border-slate-100 p-6 flex items-center justify-between bg-slate-50/50">
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 bg-primary-100 text-primary-500 rounded-xl flex items-center justify-center border border-primary-200 shadow-sm">
            ...
            <form action="{{ route('eoffice.kp.koordinator.pengaturan.store') }}" method="POST" class="p-8">
                    ...
                    <label class="flex items-center cursor-pointer group">
                        <div class="relative">
                            ...
                            <div class="block w-14 h-8 rounded-full transition-colors duration-300 shadow-inner"
                                :class="isOpen ? 'bg-primary-500' : 'bg-slate-300'"></div>
                            <div class="dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform duration-300 flex items-center justify-center shadow-md"
                                ...
                                <svg x-show="isOpen" class="w-3 h-3 text-primary-500" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                        ...
                        <div class="ml-4">
                            <p class="text-sm font-bold" :class="isOpen ? 'text-primary-500' : 'text-slate-600'">Status:
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div>
                        ...
                        <input type="date" name="pendaftaran_kp_mulai" value="{{ $startDate }}"
                            class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all font-medium text-slate-800">
                ...
                <div class="bg-primary-50 border border-primary-200 rounded-xl p-4 mb-8">
                    <div class="flex items-start">
                        <svg class="w-5 h-5 text-primary-500 mt-0.5 mr-3 flex-shrink-0" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <div>
                            <p class="text-sm text-primary-500 font-semibold mb-1">Catatan Sistem:</p>
                            <ul class="list-disc pl-5 text-xs text-primary-500 space-y-1">
                ...
                <div class="flex items-center justify-end border-t border-slate-100 pt-6 mt-6">
                    <button type="submit"
                        class="flex items-center justify-center px-6 py-2.5 bg-primary-500 text-white rounded-xl hover:bg-primary-500 transition-colors shadow-sm shadow-primary-200 font-bold text-sm focus:ring-4 focus:ring-primary-100 outline-none">
