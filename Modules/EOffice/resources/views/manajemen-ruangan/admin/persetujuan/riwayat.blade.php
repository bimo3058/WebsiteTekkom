    ...
    <x-eoffice::manajemen-ruangan.layout pageTitle="Arsip & Rekap">
        ...
        <div class="mp-page-header">
            <div>
                ...
                <p class="mp-page-sub">Gudang arsip seluruh data histori pengajuan ruangan yang telah berstatus Selesai,
                    Ditolak, atau Dibatalkan.</p>
                <h1 class="mp-page-title">Arsip & Rekap</h1>
                <p class="mp-page-sub">Gudang arsip seluruh data histori pengajuan ruangan yang telah berstatus Selesai,
                    Ditolak, atau Dibatalkan.</p>
            </div>
                                ...
                                <div class="relative" x-data="{ open: false }">
                                    <button type="button" @click="open = !open"
                                        ...
                                        @if(request()->hasAny(['status', 'ruangan_id', 'start_date', 'end_date']) && array_filter(request()->only(['status', 'ruangan_id', 'start_date', 'end_date'])))
                                            <span
                                                class="w-2 h-2 rounded-full bg-[#0B266E] absolute -top-0.5 -right-0.5"></span>
                                        @endif
                                    </button>
                                    {{-- Filter Engine Dropdown --}}
                                    <div class="relative" x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.outside="open = false" style="display: none;"
                                            ...
                                            <div class="p-4 space-y-4">
                                                ...
                                                <div>
                                                    <label
                                                        class="block text-xs font-semibold text-gray-700 mb-1.5 relative z-10 w-full bg-white">Ruangan</label>
                                                    <div x-data="{ 
                                            open: false, 
                                            ...
                                            selectItem(id, name) { 
                                                ...
                                                this.open = false; 
                                                        ...
                                                        <div x-show="open"
                                                            ...
                                                            class="absolute left-0 top-full mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg z-[60] max-h-48 overflow-y-auto"
                                                            style="display: none;">
                                                            <div class="p-1">
                                                                <button type="button"
                                                                    @click="selectItem('', 'Semua Ruangan')"
                                                                    class="w-full text-left px-3 py-2 text-xs font-medium rounded-md transition-colors cursor-pointer"
                                                                    :class="{'bg-[#EFF6FF] text-[#0B266E] font-bold': selectedId == '', 'text-gray-700 hover:bg-gray-50': selectedId != ''}">Semua
                                                                    Ruangan</button>

                                                                @foreach($ruangans as $ruangan)
                                                                    <button type="button"
                                                                        @click="selectItem('{{ $ruangan->id }}', '{{ addslashes($ruangan->nama) }}')"
                                                                        class="w-full text-left px-3 py-2 mt-0.5 text-xs font-medium rounded-md transition-colors cursor-pointer"
                                                                        :class="{'bg-[#EFF6FF] text-[#0B266E] font-bold': selectedId == '{{ $ruangan->id }}', 'text-gray-700 hover:bg-gray-50': selectedId != '{{ $ruangan->id }}'}">
                                                                        {{ $ruangan->nama }}
                                                                    </button>
                                                                @endforeach
                                                            </div>
                                                        ...
                                                        <div x-show="open"
                                                            ...
                                                            <div class="p-1">
                                                                <button type="button" @click="selectItem('')"
                                                                    ...
                                                                    Status</button>
                                                                <button type="button" @click="selectItem('disetujui')"
                                                                    class="w-full text-left px-3 py-2 mt-0.5 text-xs font-medium rounded-md transition-colors cursor-pointer"
                                                                    :class="{'bg-[#EFF6FF] text-[#0B266E] font-bold': selectedVal == 'disetujui', 'text-gray-700 hover:bg-gray-50': selectedVal != 'disetujui'}">Disetujui</button>
                                                                <button type="button" @click="selectItem('ditolak')"
                                                                    class="w-full text-left px-3 py-2 mt-0.5 text-xs font-medium rounded-md transition-colors cursor-pointer"
                                                                    :class="{'bg-[#EFF6FF] text-[#0B266E] font-bold': selectedVal == 'ditolak', 'text-gray-700 hover:bg-gray-50': selectedVal != 'ditolak'}">Ditolak</button>
                                                                <button type="button" @click="selectItem('dibatalkan')"
                                                                    class="w-full text-left px-3 py-2 mt-0.5 text-xs font-medium rounded-md transition-colors cursor-pointer"
                                                                    :class="{'bg-[#EFF6FF] text-[#0B266E] font-bold': selectedVal == 'dibatalkan', 'text-gray-700 hover:bg-gray-50': selectedVal != 'dibatalkan'}">Dibatalkan</button>
                                                                <button type="button" @click="selectItem('selesai')"
                                                                    class="w-full text-left px-3 py-2 mt-0.5 text-xs font-medium rounded-md transition-colors cursor-pointer"
                                                                    :class="{'bg-[#EFF6FF] text-[#0B266E] font-bold': selectedVal == 'selesai', 'text-gray-700 hover:bg-gray-50': selectedVal != 'selesai'}">Selesai</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                            ...
                            <div class="relative" x-data="{ openExport: false }">
                                ...
                                <div x-show="openExport" @click.outside="openExport = false" style="display: none;"
                                    ...
                                    <div class="p-2 space-y-1">
                                        <a href="{{ route('eoffice.peminjaman.admin.riwayat.export-pdf', request()->query()) }}"
                                            class="w-full text-left px-3 py-2.5 rounded-lg hover:bg-gray-50 flex items-start gap-3 transition-colors cursor-pointer">
                                            <div class="bg-red-50 p-2 rounded-md text-red-600"><svg class="w-5 h-5"
                                            ...
                                            <div>
                                                <div class="text-[13px] font-bold text-gray-800">Cetak PDF (Sesuai
                                                    Filter)</div>
                                                <div class="text-[11px] text-gray-500 mt-0.5 leading-snug">Unduh laporan
                                                    resmi format PDF sesuai data tabel saat ini</div>
                                            </div>
                                        </a>
                                        <a href="{{ route('eoffice.peminjaman.admin.riwayat.export-excel', request()->query()) }}"
                                            class="w-full text-left px-3 py-2.5 rounded-lg hover:bg-gray-50 flex items-start gap-3 transition-colors cursor-pointer">
                                            <div class="bg-green-50 p-2 rounded-md text-green-600"><svg class="w-5 h-5"
                                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2"
                                            ...
                                            <div>
                                                <div class="text-[13px] font-bold text-gray-800">Unduh Excel (Sesuai
                                                    Filter)</div>
                                                <div class="text-[11px] text-gray-500 mt-0.5 leading-snug">Unduh data
                                                    mentah Excel untuk diolah kembali</div>
                                            </div>
                                        </a>
                                        ...
                                        @endphp
                                        <a href="{{ route('eoffice.peminjaman.admin.riwayat.export-excel', ['start_date' => $startBulanIni, 'end_date' => $endBulanIni]) }}"
                                            class="w-full text-left px-3 py-2.5 rounded-lg hover:bg-gray-50 flex items-start gap-3 transition-colors cursor-pointer">
                                            <div class="bg-blue-50 p-2 rounded-md text-blue-600"><svg class="w-5 h-5"
                                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path>
                                                </svg></div>
                                            <div>
                                                <div class="text-[13px] font-bold text-gray-800">Rekap Cepat: Bulan Ini
                                                </div>
                                                <div class="text-[11px] text-gray-500 mt-0.5 leading-snug">Langsung
                                                    unduh seluruh peminjaman di bulan berjalan tanpa atur filter</div>
                                            </div>
                                        </a>
                                    </div>
                                </div>
                ...
                <div class="relative" x-data="{ openExport: false }">
                    ...
                    <div x-show="openExport" @click.outside="openExport = false" style="display: none;"
                        ...
                        <div class="p-2 space-y-1">
                            <a href="{{ route('eoffice.peminjaman.admin.riwayat.export-pdf', request()->query()) }}"
                                class="w-full text-left px-3 py-2.5 rounded-lg hover:bg-gray-50 flex items-start gap-3 transition-colors cursor-pointer">
                                <div class="bg-red-50 p-2 rounded-md text-red-600"><svg class="w-5 h-5" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z">
                                        </path>
                                ...
                                <div>
                                    ...
                                    <div class="text-[11px] text-gray-500 mt-0.5 leading-snug">Unduh laporan resmi
                                        format PDF sesuai data tabel saat ini</div>
                                </div>
                            </a>
                            <a href="{{ route('eoffice.peminjaman.admin.riwayat.export-excel', request()->query()) }}"
                                class="w-full text-left px-3 py-2.5 rounded-lg hover:bg-gray-50 flex items-start gap-3 transition-colors cursor-pointer">
                                <div class="bg-green-50 p-2 rounded-md text-green-600"><svg class="w-5 h-5" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z">
                                        </path>
                                ...
                                <div>
                                    ...
                                    <div class="text-[11px] text-gray-500 mt-0.5 leading-snug">Unduh data mentah Excel
                                        untuk diolah kembali</div>
                                </div>
                            </a>
                            <div class="border-t border-gray-100 my-1"></div>
                            @php
                            ...
                            <a href="{{ route('eoffice.peminjaman.admin.riwayat.export-excel', ['start_date' => $startBulanIni, 'end_date' => $endBulanIni]) }}"
                                class="w-full text-left px-3 py-2.5 rounded-lg hover:bg-gray-50 flex items-start gap-3 transition-colors cursor-pointer">
                                <div class="bg-blue-50 p-2 rounded-md text-blue-600"><svg class="w-5 h-5" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div>
                                    ...
                                    <div class="text-[11px] text-gray-500 mt-0.5 leading-snug">Langsung unduh seluruh
                                        peminjaman di bulan berjalan tanpa atur filter</div>
                                </div>
                            </a>
                        </div>
                    </div>
                ...
                <tbody>
                    @forelse($peminjamans as $pinjam)
                                        ...
                                        <tr class="mp-tr">
                                            <td>
                                                @if($isDosen)
                                                    <div class="text-[13px] font-semibold text-[#111827] truncate max-w-[160px]"
                                                        title="{{ $fullName }}">
                                                        {{ $fullName }}
                                                    </div>
                                                @else
                                                ...
                                                @endif
                                                <div
                                                    class="text-[11px] font-medium text-gray-500 mt-0.5 inline-block px-1.5 py-0.5 rounded {{ $isDosen ? 'bg-[#0B266E]/10 text-[#0B266E]' : 'bg-slate-100 text-slate-600' }}">
                                                    {{ $roleName }}
                                                    @php
                                                        $fullName = $pinjam->user->name ?? 'User Tidak Diketahui';
                                                        $isDosen = $pinjam->user && $pinjam->user->hasRole('dosen');
                                        ...
                                        <tr class="mp-tr">
                                            ...
                                            <td class="max-w-[140px]">
                                                <div class="text-[13px] font-medium text-[#111827] truncate"
                                                    title="{{ $pinjam->ruangan->nama ?? 'Dihapus' }}">
                                            ...
                                            <td>
                                                ...
                                                @endphp
                                                <span
                                                    style="font-size:11px; font-weight:700; color:{{ $st['color'] }}; background:{{ $st['bg'] }}; border:1px solid {{ $st['border'] }}; padding:3px 10px; border-radius:9999px; white-space:nowrap; text-transform:uppercase; display:inline-block;">
                                                    <span
                                                        style="font-size:11px; font-weight:700; color:{{ $st['color'] }}; background:{{ $st['bg'] }}; border:1px solid {{ $st['border'] }}; padding:3px 10px; border-radius:9999px; white-space:nowrap; text-transform:uppercase; display:inline-block;">
                                                        {{ $pinjam->status }}
                                                    </span>
                            ...
                            'status' => $pinjam->status
                                                    ...
                                                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-[#0B266E] hover:bg-[#0B266E]/10 hover:text-[#060E2A] transition-colors tooltip-btn cursor-pointer"
                                                    title="Lihat Detail">
                                                ...
                                                @if(auth()->user()->hasRole('superadmin'))
                                                    ...
                                                    <button type="button"
                                                        class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-red-500 hover:bg-red-50 hover:text-red-700 transition-colors tooltip-btn cursor-pointer"
                    ...
                    @empty
                        <tr class="mp-tr">
                            <td colspan="6" class="py-12 text-center text-gray-500 text-[13px]">Belum ada arsip peminjaman
                                yang tersedia.</td>
                            <td colspan="6" class="py-12 text-center text-gray-500 text-[13px]">Belum ada arsip peminjaman
                                yang tersedia.</td>
                        </tr>
            ...
            <div
                ...
                <div class="flex items-center gap-4">
                    <div
                        ...
                        <span
                            class="px-3 py-1.5 text-slate-600 font-medium border-r border-slate-200 bg-slate-50 shrink-0">Per
                            halaman</span>
                        <div x-data="{ 
                        open: false, 
                        ...
                        select(val, url) { 
                            ...
                            <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-100"
                                ...
                                class="absolute left-0 bottom-full mb-1 w-full min-w-[70px] bg-white border border-slate-200 rounded-md shadow-lg z-[60] overflow-hidden"
                                style="display: none;">
                                <div class="py-1">
                                    <button type="button"
                                        @click="select('10', '{{ request()->fullUrlWithQuery(['per_page' => 10]) }}')"
                                        class="w-full text-left px-3 py-1.5 text-slate-700 hover:bg-slate-50 hover:text-[#0B266E] font-medium transition-colors cursor-pointer"
                                        :class="{'bg-[#EFF6FF] text-[#0B266E] font-bold': value == '10'}">10</button>
                                    <button type="button"
                                        @click="select('25', '{{ request()->fullUrlWithQuery(['per_page' => 25]) }}')"
                                        class="w-full text-left px-3 py-1.5 text-slate-700 hover:bg-slate-50 hover:text-[#0B266E] font-medium transition-colors cursor-pointer"
                                        :class="{'bg-[#EFF6FF] text-[#0B266E] font-bold': value == '25'}">25</button>
                                    <button type="button"
                                        @click="select('50', '{{ request()->fullUrlWithQuery(['per_page' => 50]) }}')"
                                        class="w-full text-left px-3 py-1.5 text-slate-700 hover:bg-slate-50 hover:text-[#0B266E] font-medium transition-colors cursor-pointer"
                                        :class="{'bg-[#EFF6FF] text-[#0B266E] font-bold': value == '50'}">50</button>
                                </div>
                            </div>
                        </div>
                        <div
                        ...
                        <p class="text-xs font-medium text-slate-600">
                            Menampilkan <span
                                class="font-bold text-slate-800">{{ $peminjamans->firstItem() ?? 0 }}</span>
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        @else
                            <a href="{{ $peminjamans->previousPageUrl() }}"
                                ...
                                class="text-slate-600 hover:bg-slate-50 w-8 h-8 flex items-center justify-center rounded-md
                                border border-slate-200 bg-white shadow-sm transition-colors cursor-pointer">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M15 19l-7-7 7-7" />
                                </svg>
                        ...
                        <div
                            ...
                            @foreach ($peminjamans->getUrlRange(max(1, $peminjamans->currentPage() - 2), min($peminjamans->lastPage(), $peminjamans->currentPage() + 2)) as $page => $url)
                                @if ($page == $peminjamans->currentPage())
                                    ...
                                    class="bg-[#354371] text-white w-8 h-8 flex items-center justify-center border-r
                                    border-slate-200 transition-colors cursor-pointer">{{ $page }}</span>
                                @else
                                    <a href="{{ $url }}"
                                    ...
                                    class="text-slate-600 hover:bg-slate-50 w-8 h-8 flex items-center justify-center border-r
                                    border-slate-200 transition-colors cursor-pointer">{{ $page }}</a>
                                @endif
                            @endforeach
                        </div>
                ...
                <div x-show="isDetailOpen" style="display: none;"
                    class="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto overflow-x-hidden bg-gray-900/60 backdrop-blur-sm p-4">
                    <div x-show="isDetailOpen" @click.outside="closeDetail()"
                        ...
                        <!-- Modal Header -->
                        <div
                            class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-slate-50/50">
                            <h3 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                                <svg class="w-5 h-5 text-[#0B266E]" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="p-6 overflow-y-auto custom-scrollbar">
                            <div class="space-y-6">
                                ...
                                <div>
                                    <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">A.
                                        Identitas Peminjam</h4>
                                    <div
                                        class="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-gray-50 rounded-xl p-4 border border-gray-100">
                                        <div>
                                            <div class="text-[11px] text-gray-500 mb-0.5">Nama Lengkap</div>
                                            <div class="text-[13px] font-semibold text-gray-900"
                                                x-text="detailData.nama"></div>
                                        </div>
                                        <div>
                                            <div class="text-[11px] text-gray-500 mb-0.5">NIM / NIP</div>
                                            <div class="text-[13px] font-medium text-gray-900"
                                                x-text="detailData.nim_nip"></div>
                                        </div>
                                        <div>
                                            <div class="text-[11px] text-gray-500 mb-0.5">Nomor Telepon/WA</div>
                                            <div class="text-[13px] font-medium text-gray-900"
                                                x-text="detailData.telepon"></div>
                                        </div>
                                        <div>
                                            <div class="text-[11px] text-gray-500 mb-0.5">Role / Peran</div>
                                            <div class="inline-block px-2 py-0.5 text-[11px] font-semibold rounded bg-blue-100 text-blue-700"
                                                x-text="detailData.role"></div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">B. Detail
                                        Peminjaman</h4>
                                    <div class="bg-white rounded-xl p-4 border border-gray-200 shadow-sm space-y-4">
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <div
                                                    class="text-[13px] font-bold text-[#0B266E] flex items-center gap-1.5">
                                            ...
                                            <div>
                                                ...
                                                <div class="text-[13px] font-semibold text-gray-900"
                                                    x-text="detailData.tanggal"></div>
                                                <div class="text-[12px] text-gray-600 mt-0.5"><span
                                                        x-text="detailData.jam"></span> <span
                                                        class="text-gray-400 mx-1">•</span> <span
                                                        x-text="detailData.durasi"></span></div>
                                            </div>
                                        </div>

                                        <div class="pt-3 border-t border-gray-100">
                                            <div class="text-[11px] text-gray-500 mb-1">Kegiatan Lengkap</div>
                                            <p class="text-[13px] text-gray-800 leading-relaxed bg-gray-50 p-3 rounded-lg border border-gray-100"
                                                x-text="detailData.kegiatan"></p>
                                        </div>

                                        <template x-if="detailData.berkas">
                                            <div class="pt-3 border-t border-gray-100">
                                                <div class="text-[11px] text-gray-500 mb-1.5">Dokumen Pendukung</div>
                                                <a :href="detailData.berkas" target="_blank"
                                                    class="inline-flex items-center gap-2 px-3 py-1.5 bg-[#0B266E]/10 text-[#0B266E] hover:bg-[#0B266E]/20 rounded-lg text-xs font-semibold transition-colors border border-[#0B266E]/20 cursor-pointer">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
                                ...
                                <div>
                                    <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">C. Jejak
                                        Audit</h4>
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                            <div>
                                                ...
                                                <div class="text-[13px] font-medium text-slate-900"
                                                    x-text="detailData.waktu_pengajuan"></div>
                                            </div>
                                            <div>
                                                <div class="text-[11px] text-slate-500 mb-0.5">Waktu Diproses</div>
                                                <div class="text-[13px] font-medium text-slate-900"
                                                    x-text="detailData.waktu_diproses"></div>
                                            </div>
                                            <div>
                                                <div class="text-[11px] text-slate-500 mb-0.5">Diproses Oleh (Admin)
                                                </div>
                                                <div class="text-[13px] font-medium text-slate-900"
                                                    x-text="detailData.diproses_oleh"></div>
                                            </div>
                                        </div>

                                        <template
                                            x-if="detailData.status === 'ditolak' || detailData.status === 'dibatalkan'">
                                            <div class="mt-3 pt-3 border-t border-slate-200">
                                                <div
                                                    class="bg-red-50 border border-red-100 rounded-lg p-3 flex gap-3 items-start">
                                                    ...
                                                    <div>
                                                        <h5 class="text-[12px] font-bold text-red-800">Catatan / Alasan
                                                            Penolakan</h5>
                                                        <p class="text-[12px] text-red-700 mt-0.5 leading-relaxed"
                                                            x-text="detailData.alasan_penolakan || 'Tidak ada keterangan tambahan.'">
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
