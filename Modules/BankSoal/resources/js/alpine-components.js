...
function registerBankSoalAlpineComponents() {
  ...
  window.Alpine.data(
    "reviewEvaluation",
    ({ minimumScore = 0, totalWeight = 0, totalParameters = 0 } = {}) => ({
      score: 0,
      minimumScore,
      totalWeight,
      totalParameters,
      ...
      recalculate() {
        ...
        this.score = Array.from(checked).reduce(
          (total, input) =>
            total +
            (input.value === "1" ? Number(input.dataset.bobot || 0) : 0),
          0,
        );
      },
      ...
      get answered() {
        return (
          this.$root.querySelectorAll('input[type="radio"]:checked').length ===
          this.totalParameters
        );
      },

      get approvalDisabled() {
        return !this.answered || this.score < this.minimumScore;
      },

      prepareRevision() {
        this.$root.querySelector("#catatan").required = true;
        this.$root.querySelector("#statusReview").value = "Revisi Total";
      },

      prepareApproval() {
        if (this.approvalDisabled) return;
        this.$root.querySelector("#catatan").required = false;
        this.$root.querySelector("#statusReview").value = "Sesuai";
      },
    }),
  );
}
