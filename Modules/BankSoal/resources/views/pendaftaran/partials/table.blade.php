...
<div x-data="{
    ...
    <div x-show="selected.length > 0" x-cloak x-transition:enter="transition ease-out duration-200"
        ...
        class="mb-3 flex items-center justify-between gap-3 px-4 py-3 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <div class="flex items-center gap-2">
            {{-- Tolak --}}
            <button type="button" @click="openConfirm('{{ route('banksoal.pendaftaran.bulkReject') }}', 'POST', 'Tolak ' + countSelected + ' Peserta?', 'Data akan dihapus permanen dan tidak dapat dikembalikan.', 'bg-rose-500 hover:bg-rose-600', 'text-rose-500', 'bg-rose-50', true, selected)"
                class="inline-flex items-center gap-1.5 px-3 py-1.5 border border-rose-300 text-rose-700 hover:bg-rose-50 active:scale-95 text-xs font-bold rounded-md transition-all">
            ...
            <button type="button" @click="openConfirm('{{ route('banksoal.pendaftaran.bulkApprove') }}', 'POST', 'Setujui ' + countSelected + ' Peserta?', 'Aksi ini tidak dapat dibatalkan.', 'bg-emerald-600 hover:bg-emerald-700', 'text-emerald-600', 'bg-emerald-50', true, selected)"
                class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white text-xs font-bold rounded-md transition-all shadow-sm">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="overflow-x-auto w-full">
            <table class="w-full text-left text-[14px] text-slate-700 border-collapse">
                <thead
                    ...
                    <tr>
                        {{-- Checkbox Select All --}}
                        <th scope="col" class="px-4 py-[14px] w-10 text-center">
                            <input type="checkbox"
                                class="w-4 h-4 rounded border-slate-300 text-blue-600 cursor-pointer focus:ring-2 focus:ring-blue-500/20 focus:ring-offset-0"
                        ...
                        <th scope="col" class="px-4 py-[14px] whitespace-nowrap">Riwayat Ujian</th>
                ...
                <tbody>
                    @forelse ($pendaftars as $index => $item)
                        ...
                        <tr class="hover:bg-slate-50 transition-colors border-b border-slate-200 last:border-b-0"
                            :class="selected.includes({{ $item->id }}) ? 'bg-blue-50/60' : ''">

                            {{-- Checkbox per baris --}}
                            <td class="px-4 py-[14px] text-center">
                                @if ($isPending)
                                    <input type="checkbox"
                                        class="row-checkbox w-4 h-4 rounded border-slate-300 text-blue-600 cursor-pointer focus:ring-2 focus:ring-blue-500/20 focus:ring-offset-0"
                                        value="{{ $item->id }}" x-model="selected">
                                @else
                                    <span class="text-slate-300" title="Sudah disetujui">
                            ...
                            <td class="px-4 py-[14px] whitespace-nowrap text-slate-600 text-sm">
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    @if ($isPending)
                                        ...
                                        <button type="button" title="Tolak & Hapus"
                                            @click="openConfirm('{{ route('banksoal.pendaftaran.destroy', $item->id) }}', 'DELETE', 'Tolak & Hapus?', 'Tolak dan hapus pendaftar {{ addslashes($item->nama_lengkap) }}? Data akan dihapus dari daftar.', 'bg-rose-500 hover:bg-rose-600', 'text-rose-500', 'bg-rose-50')"
    ...
    <!-- Modal Popup: Konfirmasi Aksi Pendaftar -->
        ...
        <div x-show="confirmModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="relative w-full max-w-sm bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden max-h-full">
            ...
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl flex items-center gap-3">
                ...
                <form :action="confirmAction" method="POST" class="flex-1 m-0">
                    ...
                    <template x-if="confirmIsBulk">
                        <template x-for="id in confirmData" :key="id">
                            <input type="hidden" name="ids[]" :value="id">
                        </template>
                    </template>
                    
                    <template x-if="confirmStatus">
