<x-banksoal::layouts.admin>
    ...
    <div class="px-6 py-6 sm:px-8 sm:py-8 max-w-7xl mx-auto space-y-8">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Live Pengawasan Ujian</h1>
                <p class="text-sm text-slate-500 mt-1">Pantau mahasiswa yang sedang mengerjakan ujian secara real-time.
                </p>
            </div>
            <div class="flex gap-2">
                <button onclick="window.location.reload()"
                    class="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-lg hover:bg-slate-50 hover:text-primary transition-colors shadow-sm text-sm font-medium">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr
                            class="bg-slate-50/50 border-b border-slate-200 text-slate-500 text-xs uppercase tracking-wider">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-slate-500 text-xs">
                                                {{ $session->user->nim ?? 'NIM tidak tersedia' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <p class="text-slate-900">
                                        {{ \Carbon\Carbon::parse($session->started_at)->format('H:i:s') }}</p>
                                    <p class="text-xs text-slate-500">
                                        {{ \Carbon\Carbon::parse($session->started_at)->diffForHumans() }}</p>
                                </td>
                                <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-full bg-slate-200 rounded-full h-2 min-w-[80px]">
                                                <div class="bg-primary h-2 rounded-full" style="width: {{ $progres }}%;"></div>
                                        </div>
                                        <span
                                            class="text-xs font-medium text-slate-600">{{ $terjawab }}/{{ $totalSoal }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                    ...
                                    <div class="mt-1 text-xs text-slate-500 flex items-center gap-1">
                                        ...
                                        Tab Switches: <strong
                                            class="{{ $totalPelanggaran > 0 ? 'text-red-600 font-bold' : 'text-slate-500' }}">{{ $totalPelanggaran }}</strong>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <form action="{{ route('banksoal.admin.cbt.force-submit', $session->id) }}"
                                        method="POST"
                                        onsubmit="return confirm('Anda yakin ingin memaksa selesai sesi ujian mahasiswa ini?');">
                                        @csrf
                                        <button type="submit"
                                            class="inline-flex items-center justify-center p-2 text-red-600 hover:bg-red-50 hover:text-red-700 rounded-lg transition-colors border border-transparent hover:border-red-200"
                                            title="Force Submit">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-slate-500 text-sm font-medium">Tidak ada mahasiswa yang sedang
                                            melaksanakan ujian saat ini.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
