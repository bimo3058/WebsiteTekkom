<div style="background:#fff; border:1px solid var(--c-border); border-radius:14px; overflow:hidden; box-shadow:0 1px 3px rgba(0,0,0,.04); display:flex; flex-direction:column;">
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; padding:14px 16px; border-bottom:1px solid var(--c-border); gap:10px; flex-wrap:wrap;">
        ...
        @include('banksoal::alokasi-sesi._search_filter')
    </div>
        ...
        <table style="width:100%; border-collapse:collapse; min-width:850px;">
            ...
            <tbody>
                @forelse($jadwals as $index => $jadwal)
                    ...
                    @endphp
                    <tr style="border-bottom:1px solid #F3F4F6; transition:background .12s; cursor:pointer;" onmouseover="this.style.background='#FAFAFA'" onmouseout="this.style.background='transparent'" @click="openJadwalDrawer({{ $jadwal->id }})">
                        ...
                        <td style="padding:14px 16px;">
                            <div style="display:flex; align-items:center; gap:10px;">
                                <div style="flex:1; background:#F1F5F9; border-radius:9999px; height:6px; overflow:hidden;">
                                    <div style="height:100%; border-radius:9999px; background:{{ $isFull ? '#EF4444' : 'var(--c-primary)' }}; width: {{ min($percent, 100) }}%; transition:width 0.5s ease-out;"></div>
                                </div>
                                <span style="font-size:11px; font-weight:700; color:{{ $isFull ? '#DC2626' : 'var(--c-fg-muted)' }};">{{ $jadwal->terisi }} / {{ $jadwal->kuota }}</span>
                            </div>
                        </td>
    ...
    {{-- Pagination Area --}}
    @include('banksoal::alokasi-sesi._pagination')
</div>
