<aside class="w-72 h-full bg-white border-r border-slate-200 flex flex-col justify-between flex-shrink-0 font-inter">
    ...
    <div class="flex flex-col flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 pb-4">
        ...
        <div class="px-6 py-8 flex items-center gap-4 border-b border-transparent">
            ...
            <div class="flex flex-col">
                <span class="font-bold text-slate-900 text-[15px] leading-tight tracking-tight uppercase">GPM</span>
                <span class="font-bold text-slate-900 text-[15px] leading-tight tracking-tight uppercase">Bank Soal</span>
            </div>
        </div>
        ...
        <nav class="flex-1 px-4 space-y-1">
            ...
            <!-- Dashboard -->
            <a href="{{ route('banksoal.dashboard') }}" class="flex items-center gap-3 py-2.5 px-4 rounded-xl {{ request()->routeIs('banksoal.dashboard') && auth()->user()->hasRole('gpm') ? 'bg-[#EBF4FF] text-blue-600 font-semibold' : 'text-slate-600 hover:bg-slate-50 font-medium' }} transition-all mb-2">
            ...
            <!-- Validasi RPS -->
            <a href="{{ route('banksoal.rps.gpm.validasi-rps') }}" class="flex items-center gap-3 py-2.5 px-4 rounded-xl {{ request()->routeIs('banksoal.rps.gpm.validasi-rps*') ? 'bg-[#EBF4FF] text-blue-600 font-semibold' : 'text-slate-600 hover:bg-slate-50 font-medium' }} transition-all">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                </svg>
                <span class="text-[13px]">Validasi RPS</span>
            </a>

            <!-- Validasi Bank Soal -->
            <a href="{{ route('banksoal.soal.gpm.validasi-bank-soal') }}" class="flex items-center gap-3 py-2.5 px-4 rounded-xl {{ request()->routeIs('banksoal.soal.gpm.validasi-bank-soal*') ? 'bg-[#EBF4FF] text-blue-600 font-semibold' : 'text-slate-600 hover:bg-slate-50 font-medium' }} transition-all mt-1">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="px-6 py-4 bg-white">
        ...
        <div class="flex items-center gap-4 mb-4">
            ...
            <!-- Details -->
            <span class="font-medium text-[14px] text-slate-700 truncate">{{ auth()->user()->name ?? 'GPM' }}</span>
        </div>
