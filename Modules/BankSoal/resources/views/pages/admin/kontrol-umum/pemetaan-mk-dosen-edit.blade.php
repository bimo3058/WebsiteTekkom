    ...
    @push('scripts')
        <script>
            ...
            document.addEventListener('DOMContentLoaded', async () => {
                window.showLoader();
                try { const r = await fetch('{{ route("banksoal.api.v1.admin.pemetaan.options") }}', { headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': CSRF } }); const d = await r.json(); if (d.success) s.pool = d.data.dosen.sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true })); } catch (e) { console.error(e); }
            ...
            async function submitMapping() {
                ...
                window.showLoader();
                try {
                    const r = await fetch('{{ route("banksoal.api.v1.admin.pemetaan.dosen-mk.sync") }}', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': CSRF }, body: JSON.stringify({ mk_id: MK_ID, user_ids: [...s.sel] }) }); const d = await r.json();
