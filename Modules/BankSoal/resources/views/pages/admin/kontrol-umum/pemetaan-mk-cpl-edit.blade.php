        ...
        <div class="mapping-grid">
            <div class="panel">
                <div class="panel-header">
                    ...
                    <p>MK tidak dapat diubah di sini</p>
                </div>
    ...
    @push('scripts')
        <script>
            ...
            document.addEventListener('DOMContentLoaded', async () => {
                window.showLoader();
                try { const r = await fetch('{{ route("banksoal.api.v1.admin.pemetaan.options") }}', { headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': CSRF } }); const d = await r.json(); if (d.success) s.pool = d.data.cpl.sort((a, b) => a.kode.localeCompare(b.kode, undefined, { numeric: true })); } catch (e) { console.error(e); }
            ...
            async function submitMapping() {
                ...
                window.showLoader();
                try {
                    const r = await fetch('{{ route("banksoal.api.v1.admin.pemetaan.mk-cpl.sync") }}', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': CSRF }, body: JSON.stringify({ mk_id: MK_ID, cpl_ids: [...s.sel] }) }); const d = await r.json();
