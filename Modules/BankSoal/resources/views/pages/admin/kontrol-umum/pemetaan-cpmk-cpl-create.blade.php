<x-banksoal::layouts.admin>
    ...
    <span class="mx-2 text-slate-300">/</span>
    <span class="text-slate-800 font-semibold">Tambah Pemetaan CPMK ke CPL</span>
    @endsection
    ...
    <style>
        ...
        .pm-page .items-list{padding:10px;display:flex;flex-direction:column;gap:6px;max-height:420px;overflow-y:auto}
        .pm-page .comp-item{padding:10px 14px;border-radius:8px;border:1.5px solid var(--s200);cursor:pointer;transition:all .18s;font-size:14px;font-weight:600;color:var(--s700);user-select:none}
        .pm-page .comp-item:hover:not(.disabled):not(.selected){border-color:var(--pm);color:var(--pm);background:var(--pm-light)}
        .pm-page .comp-item.selected{background:var(--pm);border-color:var(--pm);color:#fff;box-shadow:0 2px 8px rgba(11,38,110,.2)}
        .pm-page .comp-item.disabled{background:var(--s100);border-color:var(--s200);color:var(--s400);cursor:not-allowed}
        .pm-page .sel-badge{margin:12px 14px;padding:8px 12px;background:var(--pm-light);border:1px solid rgba(11,38,110,.15);border-radius:8px;font-size:12px;color:var(--pm);font-weight:600;display:none}
    ...
    <div class="page-header">
        <div>
            <h1>Tambah Pemetaan CPMK ke CPL</h1>
            <p>Pilih satu CPL, kemudian centang CPMK yang akan dipetakan.</p>
        </div>
    ...
    <div class="mapping-grid">
        ...
        <div class="panel" id="panelLeft">
            <div class="panel-header">
                <h2>CPL Terpilih</h2>
                <p>Pilih satu CPL (maks. 1)</p>
            </div>
            <div class="panel-search">
                <div class="srch">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                    <input type="text" id="leftSearch" placeholder="Cari CPL..." oninput="onLeftSearch(this.value)">
                </div>
            </div>
            <div id="selectionBadge" class="sel-badge"></div>
            <div class="items-list" id="leftItems"></div>
            <div class="panel-footer" id="leftFooter" style="display:none;">
                <span class="pg-info" id="leftPageInfo"></span>
    ...
    <div class="form-actions">
        ...
        <button type="button" class="btn btn-primary" id="btnSimpan" onclick="submitMapping()" disabled>
    ...
    const LEFT_SIZE = 8, RIGHT_SIZE = 10;

    const s = {
        cpl: [], cpmk: [],
        selectedLeftId: null,
        selectedRightIds: new Set(),
        leftSearch: '', leftPage: 1,
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        try {
            ...
            if (d.success) {
                s.cpl  = d.data.cpl.sort((a,b)  => a.kode.localeCompare(b.kode,  undefined, {numeric:true}));
                s.cpmk = d.data.cpmk.sort((a,b) => a.kode.localeCompare(b.kode, undefined, {numeric:true}));
            }
    ...
    // ── search ────────────────────────────────────────────
    function onLeftSearch(v) { s.leftSearch = v; s.leftPage = 1; renderLeft(); }
    function onRightSearch(v) { s.rightSearch = v; s.rightPage = 1; renderRight(); }

    // ── left panel ────────────────────────────────────────
    function filteredLeft() {
        const q = s.leftSearch.toLowerCase();
        return s.cpl.filter(c => c.kode.toLowerCase().includes(q));
    }

    function renderLeft() {
        const all = filteredLeft();
        const total = all.length;
        const totalPages = Math.max(1, Math.ceil(total / LEFT_SIZE));
        if (s.leftPage > totalPages) s.leftPage = 1;
        const paged = all.slice((s.leftPage - 1) * LEFT_SIZE, s.leftPage * LEFT_SIZE);

        const container = document.getElementById('leftItems');
        if (!paged.length) {
            container.innerHTML = '<div style="padding:24px 14px;text-align:center;font-size:13px;color:#94a3b8">Tidak ada data CPL</div>';
        } else {
            const rows = paged.map(c => {
                const sel = s.selectedLeftId === c.id;
                const dis = s.selectedLeftId !== null && !sel;
                return `<div class="comp-item ${sel?'selected':''} ${dis?'disabled':''}" onclick="selectLeft(${c.id})">${c.kode}</div>`;
            }).join('');
            const ghosts = '<div class="comp-item" style="visibility:hidden;pointer-events:none">&nbsp;</div>'
                .repeat(Math.max(0, LEFT_SIZE - paged.length));
            container.innerHTML = rows + ghosts;
        }

        const badge = document.getElementById('selectionBadge');
        if (s.selectedLeftId !== null) {
            const found = s.cpl.find(c => c.id === s.selectedLeftId);
            badge.textContent = '✓ Terpilih: ' + (found ? found.kode : '');
            badge.classList.add('show');
        } else {
            badge.classList.remove('show');
        }

        renderLeftPagination(total, totalPages);
        updateSaveBtn();
    }

    function renderLeftPagination(total, totalPages) {
        const footer = document.getElementById('leftFooter');
        footer.style.display = total > LEFT_SIZE ? 'flex' : 'none';
        document.getElementById('leftPageInfo').textContent = `Hal. ${s.leftPage}/${totalPages}`;
        const pg = document.getElementById('leftPagination');
        const wS = Math.max(1, Math.min(s.leftPage - 1, totalPages - 2));
        const wE = Math.min(totalPages, wS + 2);
        let b = `<button class="pg-btn" onclick="setLeftPage(${s.leftPage-1})" ${s.leftPage<=1?'disabled':''}>‹</button>`;
        for(let p = wS; p <= wE; p++)
            b += `<button class="pg-btn ${p===s.leftPage?'active':''}" onclick="setLeftPage(${p})">${p}</button>`;
        b += `<button class="pg-btn" onclick="setLeftPage(${s.leftPage+1})" ${s.leftPage>=totalPages?'disabled':''}>›</button>`;
        pg.innerHTML = b;
    }

    function setLeftPage(p) { s.leftPage = p; renderLeft(); }

    function selectLeft(id) {
        if (s.selectedLeftId !== null && s.selectedLeftId !== id) return;
    ...
    function updateSaveBtn() {
        document.getElementById('btnSimpan').disabled = !(s.selectedLeftId !== null && s.selectedRightIds.size > 0);
    }

    // ── submit ────────────────────────────────────────────
    async function submitMapping() {
        if (s.selectedLeftId === null || s.selectedRightIds.size === 0) return;
        ...
        try {
            const r = await fetch(BASE_API + '/pemetaan/cpmk-cpl', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': CSRF },
                body: JSON.stringify({ cpl_id: s.selectedLeftId, cpmk_ids: [...s.selectedRightIds] })
            });
            const d = await r.json();
            if (r.ok && d.success) {
                ...
                btn.disabled = false;
                btn.innerHTML = '<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg> Simpan Pemetaan';
            }
        } catch (e) {
