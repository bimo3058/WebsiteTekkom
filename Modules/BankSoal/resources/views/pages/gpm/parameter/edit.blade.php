<x-banksoal::layouts.gpm-master>
    ...
    <span class="text-slate-800 font-semibold">Edit Parameter</span>
    @endsection
    ...
    <div class="max-w-3xl mx-auto py-8">
        <div class="mb-8">
            ...
            <p class="text-sm text-slate-500 mt-1">Perbarui aspek penilaian validasi yang sudah ada.</p>
        </div>

        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <form action="{{ route('banksoal.soal.gpm.parameter.update', $parameter->id) }}" method="POST" class="p-8 space-y-6" onsubmit="if(this.checkValidity()){ window.showLoader(); return true; }">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <select name="jenis" id="jenis" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all outline-none @error('jenis') border-rose-500 @enderror" required>
                            <option value="rps" {{ old('jenis', $parameter->jenis) == 'rps' ? 'selected' : '' }}>Validasi RPS</option>
                            <option value="soal" {{ old('jenis', $parameter->jenis) == 'soal' ? 'selected' : '' }}>Validasi Bank Soal</option>
                        </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <input type="number" name="bobot" id="bobot" value="{{ old('bobot', $parameter->bobot) }}" min="1" max="100" class="w-full pl-4 pr-12 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all outline-none @error('bobot') border-rose-500 @enderror" required>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="aspek" id="aspek" rows="3" placeholder="Contoh: Kesesuaian CPL dengan Materi Pembelajaran" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all outline-none @error('aspek') border-rose-500 @enderror" required>{{ old('aspek', $parameter->aspek) }}</textarea>
