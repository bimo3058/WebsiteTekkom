                ...
                <div>
                    ...
                    <div class="flex items-center gap-5 text-sm font-medium text-slate-700 mt-3">
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="tipe_soal" value="pilihan_ganda" {{ old('tipe_soal', $soal->tipe_soal ?? 'pilihan_ganda') == 'pilihan_ganda' ? 'checked' : '' }} class="w-4 h-4" onchange="toggleTipeSoal(this.value)"> 
                            Pilihan Ganda
                        </label>
                        <label class="inline-flex items-center gap-2 cursor-pointer">
            ...
            <div id="opsiContainerWrapper" class="border-t border-slate-200 px-6 py-5 {{ old('tipe_soal', $soal->tipe_soal ?? 'pilihan_ganda') == 'essay' ? 'hidden' : '' }}">
                ...
                <div id="optionsContainer" class="space-y-3">
                    ...
                    @foreach($oldJawaban as $i => $jawab)
                        <label class="option-item flex items-start gap-3 rounded-lg border p-4 {{ (string)$oldJawabanBenar === (string)$i ? 'border-green-400 bg-green-50' : 'border-slate-200' }}"><input type="radio" name="jawaban_benar" value="{{ $i }}" {{ (string)$oldJawabanBenar === (string)$i ? 'checked' : '' }} required class="mt-1 h-4 w-4 text-green-600 focus:ring-green-500"><div class="flex-1"><span class="option-title mb-1 block text-[11px] font-bold uppercase tracking-wide {{ (string)$oldJawabanBenar === (string)$i ? 'text-green-700' : 'text-slate-400' }}">Pilihan {{ chr(65 + $i) }} {{ (string)$oldJawabanBenar === (string)$i ? '(Kunci Jawaban)' : '' }}</span><input type="text" name="jawaban[{{$i}}][teks]" value="{{ $jawab['deskripsi'] ?? $jawab['teks'] ?? '' }}" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none" required></div>@if($i > 1)<button type="button" class="remove-btn text-red-500 hover:text-red-600" aria-label="Hapus opsi"><i class="fas fa-times-circle"></i></button>@endif</label>
                    @endforeach
                </div>
            </div>
