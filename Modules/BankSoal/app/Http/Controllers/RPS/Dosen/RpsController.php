...
use Modules\BankSoal\Models\RpsDraft;
...
class RpsController extends Controller
{
    ...
    public function index(): \Illuminate\View\View
    {
        ...
        $riwayat = RpsDetail::with('mataKuliah', 'dosens')
            ...
            ->get()
            ->map(function ($item) {
                // Tandai sebagai bukan draft
                $item->is_draft = false;
                $item->draft_id = null;
                return $item;
            });

        $drafts = RpsDraft::with('mataKuliah')
            ->where('user_id', $user->id)
            ->whereIn('mk_id', $mkIdsWithActiveRps->merge(
                MataKuliah::whereHas('dosenPengampu', fn($q) => $q->where('user_id', $user->id))
                    ->pluck('id')
            )->unique())
            ->orderBy('updated_at', 'desc')
            ->get()
            ->map(function ($draft) {
                // Normalisasi ke shape yang sama dengan RpsDetail
                return (object) [
                    'id' => null,
                    'is_draft' => true,
                    'draft_id' => $draft->id,
                    ...
                    'tahun_ajaran' => $draft->tahun_ajaran,
                    'creation_method' => $draft->creation_method,
                    'step_reached' => $draft->step_reached,
                    'status' => null,
                    'catatan' => null,
                    'dokumen' => null,
                    'uploader_id' => $draft->user_id,
                    'uploader_name' => null,
                    'dosens' => collect(),
        ...
        // Draft di atas, riwayat pengajuan di bawah
        $riwayat = $drafts->merge($riwayat);

        // Riwayat RPS per MK yang SUDAH disetujui — untuk seksi bawah halaman
        ...
        // Fetch Active Periode + tenggat info
        ...
        if ($activePeriode) {
            ...
            if ($now->between($start, $end)) {
                ...
                if ($deadlineDate->isSameDay($todayDate)) {
                    $hoursLeft = (int) $now->diffInHours($deadline->endOfDay());
                    if ($hoursLeft > 0) {
                        $tenggatH7 = true;
                        $daysLeft = $hoursLeft;
                        $isHourFormat = true;
    ...
    {
        try {
            // Query semua user dengan role terkait dosen via Spatie model_has_roles
            $dosenQuery = User::whereHas('roles', fn($q) => $q->whereIn('name', ['dosen', 'dosen_koor', 'gpm', 'ketua_departemen']))
                ->where('id', '!=', Auth::id())
                ->whereNull('suspended_at')
                ->orderBy('name')
                ->get();

            if ($dosenQuery->isEmpty()) {
                // Fallback: query all users except current auth user if spatie role filter returned empty
                $dosenQuery = User::where('id', '!=', Auth::id())
                    ->whereNull('suspended_at')
                    ->orderBy('name')
                    ->get();
            }
            ...
            if ($dosenQuery->isEmpty()) {
                // Last fallback: include all users
                $dosenQuery = User::whereNull('suspended_at')->orderBy('name')->get();
            }

            $dosenList = $dosenQuery->map(function ($user) {
                return [
                    'id' => $user->id,
                    'name' => $user->name,
                ];
    ...
    public function previewDokumen(int $rpsId)
    {
        try {
            ...
            if ($rps->dokumen) {
                ...
                try {
                    $response = Http::withoutVerifying()->timeout(2)->get($fileUrl);
                    if ($response->status() === 404) {
                        ...
                        $healedResponse = Http::withoutVerifying()->timeout(2)->get($healedUrl);
     ...
     */
    public function edit(int $rpsId): View
    {
        $user = Auth::user()->load('lecturer');
     ...
     */
    public function editModal(int $rpsId): View
    {
        $user = Auth::user()->load('lecturer');
        $rps = RpsDetail::with(['mataKuliah', 'cpls', 'cpmks', 'dosens'])->findOrFail($rpsId);
    ...
    public function saveDraft(Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'semester' => ['required', 'in:Ganjil,Genap'],
            'tahun_ajaran' => ['required', 'string'],
            'creation_method' => ['required', 'in:upload,generator'],
            'step_reached' => ['required', 'integer', 'min:0', 'max:3'],
            'dosen_lain' => ['nullable', 'array'],
            'dosen_lain.*' => ['exists:users,id'],
            'data_step_1' => ['nullable', 'array'],
            'data_step_2' => ['nullable', 'array'],
            'data_step_3' => ['nullable', 'array'],
        ]);

        $isDosenMk = DB::table('bs_dosen_pengampu_mk')
            ->where('user_id', $userId)
            ->where('mk_id', $validated['mk_id'])
            ->exists();

        if (!$isDosenMk) {
            return response()->json(['success' => false, 'message' => 'Anda bukan pengampu mata kuliah ini.'], 403);
        }

        $updateData = ['step_reached' => $validated['step_reached']];
        if (array_key_exists('dosen_lain', $validated))
            $updateData['dosen_lain'] = $validated['dosen_lain'];
        if (array_key_exists('data_step_1', $validated))
            $updateData['data_step_1'] = $validated['data_step_1'];
        if (array_key_exists('data_step_2', $validated))
            $updateData['data_step_2'] = $validated['data_step_2'];
        if (array_key_exists('data_step_3', $validated))
            $updateData['data_step_3'] = $validated['data_step_3'];

        $draft = RpsDraft::updateOrCreate(
            [
                'user_id' => $userId,
        ...
        return response()->json([
            ...
            'draft_id' => $draft->id,
            'step_reached' => $draft->step_reached,
            'updated_at' => $draft->updated_at->format('H:i'),
            'message' => 'Draft berhasil disimpan.',
        ]);
    }

    public function getDraft(int $mkId): JsonResponse
    {
        $drafts = RpsDraft::where('user_id', Auth::id())
            ->where('mk_id', $mkId)
            ->get();
        ...
        return response()->json([
            ...
            'data' => $drafts->map(fn($d) => [
                ...
                'step_reached' => $d->step_reached,
                'semester' => $d->semester,
                'tahun_ajaran' => $d->tahun_ajaran,
                'dosen_lain' => $d->dosen_lain ?? [],
                'data_step_1' => $d->data_step_1,
                'data_step_2' => $d->data_step_2,
                'data_step_3' => $d->data_step_3,
                'updated_at' => $d->updated_at->format('d M Y H:i'),
            ])->values(),
        ]);
    }

    public function deleteDraft(int $draftId): JsonResponse
    {
        $draft = RpsDraft::where('id', $draftId)->where('user_id', Auth::id())->first();
    ...
    {
        $request->merge(['creation_method' => 'upload']);
        return $this->store($request);
    }

    public function storeGenerator(Request $request): RedirectResponse|JsonResponse
    {
        $request->merge(['creation_method' => 'generator']);
        ...
        if ($response instanceof JsonResponse) {
            ...
            $isSuccess = !session()->has('error');
        }

        if ($isSuccess) {
            RpsDraft::where('user_id', Auth::id())
                ->where('mk_id', $request->input('mata_kuliah_id'))
                ->where('creation_method', 'generator')
                ->delete();
        }
