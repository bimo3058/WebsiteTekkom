...
@endphp
<div style="display:flex; align-items:center; justify-content:space-between; padding:12px 16px; background:var(--c-card, #fff); border-top:1px solid var(--c-border, #DFE1E7); flex-wrap:wrap; gap:10px;">
        ...
        <div style="display:flex; align-items:center; gap:4px;">
            @if($pSaatIni > 1)
                <a href="{{ $paginator->previousPageUrl() }}" aria-label="Halaman sebelumnya"
                   style="width:28px; height:28px; display:flex; align-items:center; justify-content:center; border:1px solid var(--c-border); border-radius:6px; background:var(--c-card, #fff); color:var(--c-fg-sec); text-decoration:none;">
            ...
            @else
                <span style="width:28px; height:28px; display:flex; align-items:center; justify-content:center; border:1px solid var(--c-border); border-radius:6px; background:var(--c-bg, #F6F8FA); color:var(--c-fg-placeholder, #808897); cursor:not-allowed;">
                    <svg width="13" height="13" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round"><path d="M15 18l-6-6 6-6"/></svg>
                </span>
            @endif
            ...
            @if($pMulai > 1)
                <a href="{{ $paginator->url(1) }}"
                   style="width:28px; height:28px; display:flex; align-items:center; justify-content:center; border:1px solid var(--c-border); border-radius:6px; background:var(--c-card, #fff); font-size:12px; font-weight:500; color:var(--c-fg-sec); text-decoration:none;">1</a>
                @if($pMulai > 2)
            ...
            @if($pSelesai < $pAkhir)
                ...
                <a href="{{ $paginator->url($pAkhir) }}"
                   style="width:28px; height:28px; display:flex; align-items:center; justify-content:center; border:1px solid var(--c-border); border-radius:6px; background:var(--c-card, #fff); font-size:12px; font-weight:500; color:var(--c-fg-sec); text-decoration:none;">{{ $pAkhir }}</a>
            @endif
