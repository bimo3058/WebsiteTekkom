...
<div class="poll-container"
     ...
     style="background:#fff;border:1px solid #DFE1E7;border-radius:10px;padding:14px 16px;margin-bottom:16px;box-shadow:0px 1px 2px 0px rgba(228,229,231,0.5);">

    {{-- Header --}}
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
        <div style="display:flex;align-items:center;gap:7px;">
            <div style="width:22px;height:22px;border-radius:6px;background:rgba(11,38,110,0.08);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#0B266E" stroke-width="2.5"
                     stroke-linecap="round" stroke-linejoin="round">
                    <rect x="3" y="3" width="18" height="18" rx="2"/>
                    <path d="M9 9h6M9 12h6M9 15h4"/>
                </svg>
            </div>
            <span style="font-size:12px;font-weight:700;color:#0B266E;letter-spacing:0.04em;">POLL</span>
        </div>
        <span class="poll-total-votes" style="font-size:11px;color:#666D80;font-weight:500;">
    ...
    <div class="poll-options" id="pollOptions-{{ $poll->id }}">
        @foreach($poll->options as $option)
            ...
            <div class="poll-option {{ $showResults ? 'show-result' : '' }} {{ $isVoted ? 'user-voted' : '' }}"
                 ...
                 style="position:relative;margin-bottom:7px;border-radius:8px;overflow:hidden;
                        border:1px solid {{ $isVoted ? '#0B266E' : '#DFE1E7' }};
                        background:#fff;cursor:{{ $cursor }};transition:all 0.15s;">

                {{-- Progress bar --}}
                @if($showResults)
                    <div class="poll-bar"
                         style="position:absolute;top:0;left:0;height:100%;width:{{ $pct }}%;
                                background:{{ $isVoted ? 'rgba(11,38,110,0.08)' : 'rgba(0,0,0,0.03)' }};
                                transition:width 0.6s ease;border-radius:7px 0 0 7px;pointer-events:none;">
                    </div>
                @endif
                ...
                <div style="position:relative;display:flex;align-items:center;justify-content:space-between;
                    ...
                    <div style="display:flex;align-items:center;gap:8px;">
                        @if($isVoted)
                            <svg class="poll-check-icon" width="14" height="14" viewBox="0 0 24 24" fill="none"
                                 stroke="#0B266E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                <polyline points="20 6 9 17 4 12"/>
                            </svg>
                        @elseif(!$showResults)
                            <span class="poll-radio"
                                  style="width:14px;height:14px;border-radius:50%;border:1.5px solid #DFE1E7;
                                         display:inline-block;flex-shrink:0;"></span>
                        @else
                            <span class="poll-check-icon" style="width:14px;display:inline-block;"></span>
                        @endif
                        <span class="poll-option-label"
                              style="font-size:13px;font-weight:{{ $isVoted ? '700' : '500' }};
                                     color:{{ $isVoted ? '#0B266E' : '#353849' }};">
                            {{ $option->text }}
                        </span>
                    </div>
                    <span class="poll-pct"
                          style="font-size:12px;font-weight:700;white-space:nowrap;margin-left:8px;
                                 color:{{ $isVoted ? '#0B266E' : '#666D80' }};
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-top:6px;flex-wrap:wrap;gap:6px;">
        <span class="poll-status-label" style="font-size:11px;color:#666D80;">
            @if($isClosed)
                <span class="poll-closed-badge" style="background:#FADAE1;color:#DF1C41;padding:2px 8px;border-radius:8px;font-weight:600;font-size:10px;">
                    Poll ditutup
                </span>
                @if($poll->expires_at)
        ...
        <div style="display:flex;align-items:center;gap:8px;">
            @if($hasVoted && !$isClosed)
                <span class="poll-change-hint" style="font-size:11px;color:#666D80;">
            ...
            @if($isOwner)
                <button class="poll-toggle-btn"
                        ...
                        style="font-size:11px;font-weight:600;padding:3px 10px;border-radius:8px;border:1px solid;cursor:pointer;background:transparent;transition:all 0.15s;
                               {{ $isClosed ? 'color:#287F6E;border-color:#287F6E;' : 'color:#DF1C41;border-color:#DF1C41;' }}">
...
<style>
    ...
    .poll-container[data-is-open="1"] .poll-option:hover {
        border-color: #5C78B8 !important;
        background: rgba(11,38,110,0.03) !important;
    }
        ...
        btn.addEventListener('click', function () {
            ...
            .then(data => {
                ...
                if (nowClosed) {
                    ...
                    btn.style.color = '#287F6E';
                    btn.style.borderColor = '#287F6E';
                } else {
        ...
        el.addEventListener('click', function () {
            ...
            .then(data => {
                ...
                container.querySelectorAll('.poll-option').forEach(function (optEl) {
                    ...
                    // Border
                    optEl.style.border = '1px solid ' + (isVoted ? '#0B266E' : '#DFE1E7');
                    ...
                    bar.style.background = isVoted ? 'rgba(11,38,110,0.08)' : 'rgba(0,0,0,0.04)';

                    // Icon: centang jika voted, spacer jika tidak
                    const iconEl = optEl.querySelector('.poll-check-icon');
                    if (iconEl) {
                        iconEl.outerHTML = isVoted
                            ? `<svg class="poll-check-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#0B266E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>`
                    ...
                    if (labelEl) {
                        ...
                        labelEl.style.color      = isVoted ? '#0B266E' : '#353849';
                    }
                    ...
                    if (pctEl) {
                        ...
                        pctEl.style.color     = isVoted ? '#0B266E' : '#666D80';
                        pctEl.style.display   = '';
                    }
                });
            })
