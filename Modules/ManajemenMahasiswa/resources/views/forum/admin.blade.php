<x-manajemenmahasiswa::layouts.admin>
    ...
    @push('scripts')
        ...
        <script>
            ...
            function getPersonalPins() {
                try { return JSON.parse(localStorage.getItem(PERSONAL_PINS_KEY)) || []; }
                catch { return []; }
            }
            ...
            function refreshPersonalPinUI() {
                ...
                // Update badges
                document.querySelectorAll('[data-personal-pin]').forEach(badge => {
                    const tid = parseInt(badge.dataset.personalPin);
                    badge.style.display = pins.includes(tid) ? 'inline-flex' : 'none';
                });
                // Update dropdown labels
                document.querySelectorAll('.personal-pin-label').forEach(label => {
                    const tid = parseInt(label.dataset.threadId);
                    label.textContent = pins.includes(tid) ? 'Unpin Pribadi' : 'Pin Pribadi';
                });
                // Sort: move personal-pinned cards to top (after global pinned)
                const container = document.querySelector('.forum-cards-container');
                if (!container) return;
                const cards = [...container.querySelectorAll('.forum-card-wrapper')];
                cards.sort((a, b) => {
            ...
            // Bind toggle buttons
            document.querySelectorAll('.personal-pin-toggle').forEach(btn => {
                btn.addEventListener('click', function(e) {
                    e.preventDefault();
                    togglePersonalPin(parseInt(this.dataset.threadId));
                });
            });
