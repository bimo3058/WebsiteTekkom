...
<div class="d-flex justify-content-between align-items-start mb-4">
    <div class="d-flex align-items-center gap-3">
        <a href="{{ route('manajemenmahasiswa.persuratan.index') }}" class="btn-back">&larr;</a>
        <div>
            <h3 class="fw-bold mb-0 text-dark">Detail Persuratan</h3>
            <p class="text-muted mb-0" style="font-size:14px;font-weight:500;">Proses Persuratan &amp; Tanda Tangan</p>
        </div>
    </div>
...
<div class="detail-card" style="padding:20px 24px;">
    ...
    <div class="d-flex align-items-center gap-3 flex-wrap">
        ...
        @if($proker->status === 'draft' && ($isCreator || $isPengurus))
            <form action="{{ route('manajemenmahasiswa.persuratan.ajukan', $proker->id) }}" method="POST" enctype="multipart/form-data" style="display:inline;">
                @csrf @method('PATCH')
                <label style="display:inline-flex;align-items:center;gap:8px;padding:8px 16px;background:#f0f9ff;border:1px solid #bae6fd;border-radius:10px;cursor:pointer;font-size:13px;font-weight:600;color:#0369a1;margin-right:8px;">
                    ...
                    <span id="suratPdfLbl">{{ $proker->surat_proker ? 'Ganti PDF Surat' : 'Upload PDF Surat' }}</span>
                    <input type="file" name="surat_proker" accept=".pdf" style="display:none;" onchange="document.getElementById('suratPdfLbl').textContent=this.files[0]?.name??'PDF dipilih'">
                </label>
                <button type="submit" class="btn-ajukan">
                    @if(!$proker->surat_proker)
                        🔒 Ajukan (Upload PDF dulu)
                    @else
...
<div class="detail-card">
    ...
    <div style="margin-top:20px;border-top:1px solid #f3f4f6;padding-top:20px;">
        ...
        <div style="background:#dcfce7;border:1px solid #bbf7d0;border-radius:10px;padding:14px;display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;">
            ...
            <button type="button"
                onclick="batalkanTtd('{{ $myRole }}', {{ $proker->id }}, '{{ route('manajemenmahasiswa.persuratan.batal_ttd', $proker->id) }}')"
        ...
        {{-- Tombol Pasang TTD --}}
        <div style="margin-top:16px;">
            <button class="btn-pasang-ttd" id="btnPasangTtd" onclick="submitTtd('{{ $myRole }}', {{ $proker->id }}, '{{ route('manajemenmahasiswa.persuratan.pasang_ttd', $proker->id) }}')" disabled>
...
<div id="tolakModal" class="modal-overlay" style="display:none;">
    <div class="modal-box">
        ...
        <form action="{{ route('manajemenmahasiswa.persuratan.tolak', $proker->id) }}" method="POST">
            @csrf @method('PATCH')
