...
<style>
    ...
    .settings-nav-btn:hover  { background: #f8fafc; color: #1e293b; }
    .settings-nav-btn.active { background: #f1f5f9; color: #0f172a; font-weight: 600; }
    .settings-nav-btn .material-symbols-outlined { font-size: 18px; }
    ...
    .panel-title { font-size: 15px; font-weight: 700; color: #0f172a; margin-bottom: 2px; }
    .panel-sub   { font-size: 13px; color: #64748b; margin-bottom: 24px; }
    ...
    .field-input:focus  { border-color: #5E53F4; box-shadow: 0 0 0 3px rgba(94,83,244,.08); }
    .field-input.readonly { background: #f8fafc; color: #94a3b8; cursor: not-allowed; }

    .toggle-track { width: 40px; height: 22px; border-radius: 99px; display: flex; align-items: center; padding: 2px; }
    .toggle-track.on  { background: #1E1B4B; }
    .toggle-track.off { background: #cbd5e1; }
    .toggle-thumb { width: 18px; height: 18px; border-radius: 50%; background: #fff; box-shadow: 0 1px 3px rgba(0,0,0,.2); transition: transform .2s; }
    .toggle-track.on .toggle-thumb { transform: translateX(18px); }

    .str-bar { height: 4px; flex: 1; border-radius: 99px; background: #e2e8f0; transition: background .3s; }
</style>

<div class="settings-wrap" x-data="{ tab: '{{ session('status') === 'password-updated' ? 'password' : 'general' }}' }">
        ...
        <div class="settings-box-header">
            <div style="display:flex;align-items:center;justify-content:space-between;">
                <div style="display:flex;align-items:center;gap:10px;">
                    <span class="material-symbols-outlined" style="font-size:20px;color:#5E53F4;">settings</span>
