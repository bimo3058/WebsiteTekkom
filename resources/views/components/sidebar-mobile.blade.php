...
<div x-data="{ show: window.innerWidth < 768, openMenu: false, startY: 0 }"
    ...
    <div x-show="openMenu" class="fixed inset-0 z-[60] font-['Inter_Tight']" style="display: none;">
        ...
        <div x-show="openMenu" x-transition:enter="transition ease-out duration-300 transform"
            ...
            <div class="flex items-center gap-3 px-5 pb-4 border-b border-slate-100 shrink-0">
                <div
                    class="w-11 h-11 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-sm overflow-hidden border-2 border-white shadow-sm {{ $rc }}">
                    @if($user->avatar_url)
                    ...
                    <div class="space-y-1">
                        @if($isSuperadmin || $isDosen || $isMahasiswa)
                            <a href="{{ $isMahasiswa ? route('komprehensif.mahasiswa.dashboard') : route('banksoal.dashboard') }}"
                                ...
                                <svg class="w-5 h-5 flex-shrink-0 {{ str_contains($currentRoute, 'banksoal') || str_contains($currentRoute, 'komprehensif') ? 'text-[#5E53F4]' : 'text-slate-400' }}"
                                ...
                                <span
                                    class="text-[13px] font-semibold">{{ $isMahasiswa ? 'Uji Komprehensif' : 'Bank Soal' }}</span>
                            </a>
                        @endif

                        <a href="{{ route('capstone.dashboard') }}" @click="openMenu = false"
                            ...
                            <svg class="w-5 h-5 flex-shrink-0 {{ str_contains($currentRoute, 'capstone') ? 'text-[#5E53F4]' : 'text-slate-400' }}"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <a href="{{ route('manajemenmahasiswa.dashboard') }}" @click="openMenu = false"
                            ...
                            <svg class="w-5 h-5 flex-shrink-0 {{ str_contains($currentRoute, 'manajemen-mahasiswa') ? 'text-[#5E53F4]' : 'text-slate-400' }}"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                            </svg>
                            <span
                                class="text-[13px] font-semibold">{{ $isMahasiswa ? 'Kemahasiswaan' : 'Manajemen Mahasiswa' }}</span>
                        </a>
                        ...
                        <a href="{{ route('eoffice.dashboard') }}" @click="openMenu = false"
                            ...
                            <svg class="w-5 h-5 flex-shrink-0 {{ str_contains($currentRoute, 'eoffice') ? 'text-[#5E53F4]' : 'text-slate-400' }}"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                                <path stroke-linecap="round" stroke-linejoin="round"
                ...
                @if($isSuperadmin)
                    <div>
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 px-1 opacity-80">
                            Sistem Ekstra</p>
                        <div class="space-y-1">
                            <a href="{{ route('superadmin.modules') }}" @click="openMenu = false"
                                ...
                                <svg class="w-5 h-5 flex-shrink-0 {{ str_contains($currentRoute, 'modules') ? 'text-[#5E53F4]' : 'text-slate-400' }}"
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div>
                    ...
                    <div class="space-y-1">
                        <a href="{{ route('profile.edit') }}" @click="openMenu = false"
                            ...
                            <svg class="w-5 h-5 flex-shrink-0 {{ str_contains($currentRoute, 'profile') ? 'text-[#5E53F4]' : 'text-slate-400' }}"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                            </svg>
    ...
    <div class="flex items-center justify-around px-2 py-1.5 safe-area-pb bg-white relative z-10">
        ...
        {{-- 1. Dashboard --}}
        <a href="{{ $isSuperadmin ? route('superadmin.dashboard') : route('dashboard') }}" class="flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-all
               {{ str_contains($currentRoute, 'dashboard') ? 'text-[#5E53F4]' : 'text-[#ADB5BD]' }}">
            <svg class="w-5 h-5 {{ str_contains($currentRoute, 'dashboard') ? 'text-[#5E53F4]' : 'text-slate-400' }}"
                fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                <path stroke-linecap="round" stroke-linejoin="round"
        ...
        @if($isSuperadmin)
            {{-- 2. User Management --}}
            <a href="{{ route('superadmin.users.index') }}" class="flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-all
                       {{ str_contains($currentRoute, 'users') ? 'text-[#5E53F4]' : 'text-[#ADB5BD]' }}">
                <svg class="w-5 h-5 {{ str_contains($currentRoute, 'users') ? 'text-[#5E53F4]' : 'text-slate-400' }}"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                </svg>
            ...
            {{-- 3. Permissions --}}
            <a href="{{ route('superadmin.permissions') }}" class="flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-all
                       {{ str_contains($currentRoute, 'permissions') ? 'text-[#5E53F4]' : 'text-[#ADB5BD]' }}">
                <svg class="w-5 h-5 {{ str_contains($currentRoute, 'permissions') ? 'text-[#5E53F4]' : 'text-slate-400' }}"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                    <path stroke-linecap="round" stroke-linejoin="round"
            ...
            {{-- 4. Audit Logs --}}
            <a href="{{ route('superadmin.audit-logs') }}" class="flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-all
                       {{ str_contains($currentRoute, 'audit-logs') ? 'text-[#5E53F4]' : 'text-[#ADB5BD]' }}">
                <svg class="w-5 h-5 {{ str_contains($currentRoute, 'audit-logs') ? 'text-[#5E53F4]' : 'text-slate-400' }}"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                    <path stroke-linecap="round" stroke-linejoin="round"
...
<div x-data="{ show: window.innerWidth < 768 }"
    x-init="window.addEventListener('resize', () => show = window.innerWidth < 768)" x-show="show"
    style="display: none;" class="h-16"></div>