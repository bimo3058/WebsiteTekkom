@props(['icon', 'label', 'route' => '#', 'active' => false, 'color' => 'text-white'])

<a href="{{ $route }}" 
   {{ $attributes->merge(['class' => 'flex items-center px-2 py-2 rounded-lg transition-colors ' . ($active ? 'bg-slate-700/50 text-white' : 'text-slate-300 hover:text-white hover:bg-slate-700/30')]) }}
