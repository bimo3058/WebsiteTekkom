...
<div x-data="{ open: localStorage.getItem('sidebarOpen') !== 'false' }"
    ...
    <aside :class="open ? 'is-open' : 'is-collapsed'" class="sitkom-sidebar">
        ...
        <nav class="sb-nav">
            ...
            @if($isSuperadmin)
                <x-sidebar-link :href="route('superadmin.dashboard')" :icon="$iconDashboard"
                    label="Dashboard" :active="str_contains($currentRoute, 'dashboard')"/>
            @else
                <x-sidebar-link :href="route('dashboard')" :icon="$iconDashboard"
                    label="Dashboard" :active="$currentRoute === 'dashboard'"/>
            @endif

            {{-- KENDALI --}}
            @if($isSuperadmin)
                <div x-show="open" class="sb-section-label">Kendali</div>
                <x-sidebar-link :href="route('superadmin.users.index')" :icon="$iconUsers"
                    label="User Management" :active="str_contains($currentRoute, 'users')"/>
                <x-sidebar-link :href="route('superadmin.permissions')" :icon="$iconShield"
                    label="Permissions" :active="str_contains($currentRoute, 'permissions')"/>
                <x-sidebar-link :href="route('superadmin.audit-logs')" :icon="$iconAuditLog"
            ...
            <x-sidebar-link :href="route('banksoal.dashboard')" :icon="$iconBankSoal"
                label="SIBASO" :active="str_contains($currentRoute, 'banksoal')"/>
            <x-sidebar-link :href="route('capstone.dashboard')" :icon="$iconCapstone"
                label="SICATA" :active="str_contains($currentRoute, 'capstone')"/>
            <x-sidebar-link :href="route('manajemenmahasiswa.dashboard')" :icon="$iconSimenma"
                label="SIMENMA" :active="str_contains($currentRoute, 'manajemen-mahasiswa')"/>
            <x-sidebar-link :href="route('eoffice.dashboard')" :icon="$iconClock"
                label="SIPERKOM" :active="str_contains($currentRoute, 'eoffice')"/>

            {{-- SETTING --}}
            @if($isSuperadmin)
                <div x-show="open" class="sb-section-label">Setting</div>
                <x-sidebar-link :href="route('superadmin.modules')" :icon="$iconGear"
                    label="Modul Setting" :active="str_contains($currentRoute, 'modules')"/>
                <x-sidebar-link href="/pulse" :icon="$iconMonitor"
                    label="System Monitor" target="_blank" :active="request()->is('pulse*')"/>
            @endif
        </nav>

        {{-- Footer --}}
        <div class="sb-footer">
            <x-sidebar-link :href="route('profile.edit')" :icon="$iconGear"
                label="Settings" :active="str_contains($currentRoute, 'profile')"/>
...
.sitkom-sidebar.is-open      { width: 240px; }
.sitkom-sidebar.is-collapsed { width: 64px; }
...
.sb-link:hover { background: var(--c-bg); }
.sb-link svg { width: 16px; height: 16px; color: var(--c-fg-muted); flex-shrink: 0; }
.sb-link-danger { color: var(--c-error); }
.sb-link-danger svg { color: var(--c-error); }
.sb-link-danger:hover { background: #FEF1F4; }
...
.sitkom-topbar-meta { line-height: 1.2; }
...
/* Content */
.sitkom-content { padding: 24px 28px 48px; }
</style>