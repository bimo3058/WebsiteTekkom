...
<div class="mod-card" style="background:#fff; border:1px solid var(--c-border); border-radius:14px; overflow:hidden; box-shadow:0px 1px 2px rgba(228,229,231,0.24); transition:border-color .15s, box-shadow .15s; display:flex; flex-direction:column;"
    ...
    <div style="padding:16px;">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:14px;">
            ...
            <div style="display:flex; flex-direction:column; align-items:flex-end; gap:4px;">
                ...
                <form action="{{ route('superadmin.modules.toggle', $module->slug) }}" method="POST">
                    @csrf
                    <button type="submit"
                            role="switch" aria-checked="{{ $isOn ? 'true' : 'false' }}" aria-label="Status modul {{ $module->name }}" title="{{ $isOn ? 'Nonaktifkan' : 'Aktifkan' }}"
        ...
        {{-- Action button --}}
        <button type="button" class="mod-manage" onclick="openModal('modal-{{ $module->slug }}')"
