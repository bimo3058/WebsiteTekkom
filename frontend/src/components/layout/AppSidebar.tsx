  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (saved && saved[sectionId] !== undefined) {
        // Restore the value maintained by the external localStorage store.
...
export function AppSidebar() {
  ...
  useEffect(() => {
    ...
    if (activeCategory) {
      // Synchronize expanded navigation with the external pathname state.
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setOpenCategory(
      ...
      if (effectiveRole === "dosen") {
        try {
          const res = await api.get(
            "/dosen/supervisor-evaluation/pending-count"
          );
    ...
    const fetchExaminerEvalCount = async () => {
      ...
      if (effectiveRole === "dosen") {
        try {
          ...
          const res = await api.get(
            "/dosen/seminar-schedules/examiner"
          );
