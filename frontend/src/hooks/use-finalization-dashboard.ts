...
export function useFinalizationDashboard(
  ...
  useEffect(() => {
    if (dashboardQuery.isFetching) {
      // Mirror the external query state in the user-facing error state.
  ...
  useEffect(() => {
    if (!selectedPeriodId) {
      // Keep the selector synchronized with the selected-period state.
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setShowPeriodSelector(true);
    }
  }, [selectedPeriodId]);
  ...
  useEffect(() => {
    if (dashboardQuery.isSuccess && selectedPeriodId) {
      // Close the selector once the external query has completed.
      // eslint-disable-next-line react-hooks/set-state-in-effect
