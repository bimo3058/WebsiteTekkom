...
# Fix search bar inline padding
if 'class="mp-input w-full pl-9 pr-3"' in text:
    text = text.replace('class="mp-input w-full pl-9 pr-3" style="height:36px; font-size:13px; width:250px;"', 
                        'class="mp-input w-full" style="height:36px; font-size:13px; width:250px; padding-left: 36px; padding-right: 12px;"')

# Now let's fix the table body.
# We will just write a regex to replace the entire <tr> inside the @forelse
pattern = r'(@forelse\(\ as \\)\s*)(<tr.*?</tr>)(\s*@empty)'
match = re.search(pattern, text, re.DOTALL)
if match:
    prefix = match.group(1)
    suffix = match.group(3)
    
    # We construct the new <tr> entirely!
    new_tr = '''<tr class="mp-tr" style="border-bottom:1px solid #DFE1E7;">
    ...
    text = text[:match.start()] + prefix + new_tr + suffix + text[match.end():]
    print("Successfully replaced table row!")
else:
    print("Could not match regex for table row!")
