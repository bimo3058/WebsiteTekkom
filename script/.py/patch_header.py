...
# Match the old sec-head and the old global filter bar
pattern = r'<div class="sec-head" style="margin-top: 24px;">.*?<div x-data="\{ globalSearch: \'\', globalKelompok: \'\', globalShift: \'\' \}">\s*\{\{-- Global Filter Bar --\}\}.*?</a>\s*</div>'

new_bar_html = """<div x-data="{ globalSearch: '', globalKelompok: '', globalShift: '' }">
    <div class="mp-card" style="margin-bottom:24px; padding:16px 24px; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:16px;">
        <div style="font-size:16px; font-weight:700; color:#0D0D12; white-space:nowrap;">Absensi & Nilai</div>
        ...
        <div style="display:flex; align-items:center; gap:12px; flex-wrap:wrap; flex:1; justify-content:flex-end;">
            ...
            <select x-model="globalKelompok" class="mp-input" style="width:160px; height: 38px; flex-shrink:0;">
            ...
            <select x-model="globalShift" class="mp-input" style="width:150px; height: 38px; flex-shrink:0;">
            ...
            <div style="position:relative; flex:1; max-width:260px; min-width:180px;">
                ...
                <input type="text" x-model="globalSearch" placeholder="Cari nama / NIM..." class="mp-input" style="width:100%; padding-left:36px; height: 38px;">
            </div>
        </div>
    </div>"""

text = re.sub(pattern, new_bar_html, text, flags=re.DOTALL)

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(text)

print("Title and Filter bar combined")
