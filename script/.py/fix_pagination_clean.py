...
old_pagination = """    @if($pendaftaran->hasPages())
    <div style="padding:12px 16px;border-top:1px solid #DFE1E7;flex-shrink:0;">{{ $pendaftaran->links() }}</div>
    @endif"""

new_pagination = """    @if(isset($pendaftaran) && method_exists($pendaftaran, 'hasPages') && ($pendaftaran->hasPages() || $pendaftaran->total() > 0))
...
if old_pagination in text:
    text = text.replace(old_pagination, new_pagination)
else:
    print('COULD NOT FIND REGULAR PAGINATION!')

with open('Modules/EOffice/resources/views/manajemen-praktikum/dosen/pendaftaran-koor.blade.php', 'w', encoding='utf-8') as f:
    f.write(text)

print('Pagination is NOW PERFECT!')